...
parser.add_argument("--is-external", action="store_const", const=True, default=False)

parser.add_argument("--id", type=string, default=None, help="package app id")
...
def main(args):
    ...
    if args.action in [
        ...
        if args.id is None:
            raise SlimArgumentError(
                "The remove action requires and app id, use the --id parameter to set this."
            )
    ...
    SlimLogger.information(
        "Saved updated installation graph to ", encode_filename(filename)
    )


def update_installation(
    installation_graph, action_list, target_os, disable_automatic_resolution=False
):
    ...
    with stream_type(value=installation_graph) as istream:
        ...
        for action in action_list:
            ...
            if workloads:
                ...
                action_item.args.workloads = (
                    AppDeploymentSpecification.from_forwarder_workloads(workloads)
                )
        ...
        server_collection = _update_installation(
            istream,
            slim_configuration.repository_path,
            actions,
            target_os,
            disable_automatic_resolution,
        )
...
def _update_installation(
    file, repository, actions, target_os, disable_automatic_resolution
):
    ...
    for action in actions:
        server_collection.update_installation(
            action, target_os, disable_automatic_resolution
        )
        SlimLogger.exit_on_error()
