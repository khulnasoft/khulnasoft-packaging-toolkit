...
#
# There is a default segmenters.conf in $KHULNASOFT_HOME/etc/system/default. To set
# custom configurations, place a segmenters.conf in
# $KHULNASOFT_HOME/etc/system/local/.  For examples, see segmenters.conf.example.
...
* Follow this stanza name with any number of the following attribute/value
  pairs.
* If you don't specify an attribute/value pair, Khulnasoft will use the default.
...
* In addition to the segments specified by the major breakers, for each minor
  breaker found, Khulnasoft indexes the token from the last major breaker to the
...
LOOKAHEAD = <integer>
* Set how far into a given event (in characters) Khulnasoft segments.
