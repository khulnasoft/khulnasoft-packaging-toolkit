...
#
# There is a transactiontypes.conf in $KHULNASOFT_HOME/etc/system/default/.  To set
# custom configurations, place a transactiontypes.conf in
# $KHULNASOFT_HOME/etc/system/local/. You must restart Khulnasoft to enable
# configurations.
#
...
* Use the stanza name, [<TRANSACTIONTYPE>], to search for the transaction in
  Khulnasoft Web.
* If you do not specify an entry for each of the following attributes, Khulnasoft
...
* Specifies the maximum number of not yet closed transactions to keep in the
  open pool. When this limit is surpassed, Khulnasoft begins evicting transactions
...
* Specifies the maximum number of events (can be) part of open transactions.
  When this limit is surpassed, Khulnasoft begins evicting transactions using LRU
