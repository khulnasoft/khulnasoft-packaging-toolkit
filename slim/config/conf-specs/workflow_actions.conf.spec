...
# actions in Khulnasoft.
#
# There is a workflow_actions.conf in $KHULNASOFT_HOME/etc/apps/search/default/.
# To set custom configurations, place a workflow_actions.conf in either
# $KHULNASOFT_HOME/etc/system/local/ or add a workflow_actions.conf file to your
# app's local/ directory. For examples, see workflow_actions.conf.example.
# You must restart Khulnasoft to enable configurations, unless editing them
# through the Khulnasoft manager.
#
...
* If not set, Khulnasoft skips this workflow action.
...
search.app = <string>
* The name of the Khulnasoft application in which to perform the constructed
  search.
...
* Accepts absolute and Khulnasoft relative times (e.g. -10h).
