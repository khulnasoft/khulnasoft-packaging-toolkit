...
#  * Telling Khulnasoft how to handle multi-value fields.
...
#
# There is a fields.conf in $KHULNASOFT_HOME/etc/system/default/.  To set custom
# configurations, place a fields.conf in $KHULNASOFT_HOME/etc/system/local/.  For
# examples, see fields.conf.example.  You must restart Khulnasoft to enable
# configurations.
#
...
* For both substitution constructs, if the resulting string starts with a '[',
  Khulnasoft interprets the string as a Khulnasoft LISPY expression.  For example,
