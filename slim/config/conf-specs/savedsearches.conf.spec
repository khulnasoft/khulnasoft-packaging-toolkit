...
# savedsearches.conf.
#
# There is a default savedsearches.conf in $KHULNASOFT_HOME/etc/system/default. To
# set custom configurations, place a savedsearches.conf in
# $KHULNASOFT_HOME/etc/system/local/.  For examples, see
# savedsearches.conf.example. You must restart Khulnasoft to enable configurations.
#
...
* Khulnasoft recommends that you schedule your searches so that they are staggered
...
* Khulnasoft's cron implementation does not currently support names of months/days.
* Defaults to empty string.
...
* When configured in Khulnasoft Web, the following email settings
...
* Defaults to khulnasoft@<LOCALHOST> (or whatever is set in alert_actions.conf).

action.email.subject = <string>
* Set the subject of the email delivered to recipients.
* Defaults to KhulnasoftAlert-<savedsearchname> (or whatever is set
...
* Can be one of the following two options:
  ...
  * A path to a lookup .csv file that Khulnasoft should copy the search results to,
    relative to $KHULNASOFT_HOME.
...
* Toggles whether this search runs when Khulnasoft starts or any edit that changes
...
* If the integer is followed by the letter 'p' Khulnasoft interprets the ttl as a
  ...
  set to 2 hours).
* If an action is triggered Khulnasoft changes the ttl to that action's ttl. If
  multiple actions are triggered, Khulnasoft applies the largest action ttl to the
...
* Defines the time format that Khulnasoft uses to specify the earliest and latest
  time.
* Defaults to %FT%T.%Q%:z

dispatch.spawn_process = 1 | 0
* Specifies whether Khulnasoft spawns a new search process when this saved search
  is executed.
* Default is 1.
...
* Specifies how frequently Khulnasoft should run the MapReduce reduce phase on
...
* For example: 60 = 60 seconds, 1m = 1 minute, 1h = 60 minutes = 1 hour etc
* Defaults to 24h.
* This property is valid until khulnasoftd restarts. Restart clears the listing of
  triggered alerts.

alert.digest_mode = true | false
* Specifies whether Khulnasoft applies the alert actions to the entire result set
...
* Specifies a field used by Khulnasoft UI to denote the app this search should be
  dispatched in.
* Defaults to empty string.

request.ui_dispatch_view = <string>
* Specifies a field used by Khulnasoft UI to denote the view this search should be
  displayed in.
* Defaults to empty string.
...
* not used anymore, the Khulnasoft software computes this value during runtime
