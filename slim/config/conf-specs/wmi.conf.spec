...
# Management Instrumentation (WMI) access from Khulnasoft.
#
# There is a wmi.conf in $KHULNASOFT_HOME\etc\system\default\.  To set custom
# configurations, place a wmi.conf in $KHULNASOFT_HOME\etc\system\local\. For
...
* If the stanza is missing, Khulnasoft assumes system defaults.
...
* Once max_backoff is reached, tells Khulnasoft how many times to attempt
  to reconnect to the WMI provider.
* Khulnasoft will try to reconnect every max_backoff seconds.
...
* Tells Khulnasoft to expect event log data for this stanza, and specifies
  the event log channels you want Khulnasoft to monitor.
...
wql = <string>
* Tells Khulnasoft to expect data from a WMI provider for this stanza, and
  specifies the WQL query you want Khulnasoft to make to gather that data.
...
* For example,
  SELECT * FROM Win32_PerfFormattedData_PerfProc_Process WHERE Name = "khulnasoftd".
