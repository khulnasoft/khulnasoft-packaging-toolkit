...
* Khulnasoft does not expand macros when they are inside of quoted values, for
  example: "foo`bar`baz".
...
* Khulnasoft replaces the $<arg>$ pattern globally in the string, even inside of
  quotes.
...
* If the validation expression is boolean, validation succeeds when it returns
  true.  If it returns false or is NULL, validation fails, and Khulnasoft returns
  the error message defined by the attribute, errormsg.
* If the validation expression is not boolean, Khulnasoft expects it to return a
