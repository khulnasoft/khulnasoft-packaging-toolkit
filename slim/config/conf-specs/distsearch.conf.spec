...
# $KHULNASOFT_HOME/etc/system/local/.  For examples, see distsearch.conf.example.
...
* Follow this stanza name with any number of the following attribute/value
  pairs.
* If you do not set any attribute, Khulnasoft uses the default value (if there
  is one listed).
...
* A user may use the khulnasoft_server arguments to target a search to qurantined peers
...
certDir = <directory>
* This directory contains the local Khulnasoft instance's distributed search key
  pair.
* This directory also contains the public keys of servers that distribute
  searches to this Khulnasoft instance.

publicKey = <filename>
* Name of public key file for this Khulnasoft instance.

privateKey = <filename>
* Name of private key file for this Khulnasoft instance.
...
* Any individual file within a bundle that is larger than this value (in MB)
  will trigger a khulnasoftd.log message.
...
* If set to auto, streaming bundle replication will be used when connecting to
  peers with a complete implementation of this feature (Khulnasoft 6.0 or higher).
* If set to true, streaming bundle replication will be used when connecting to
  peers with a complete or experimental implementation of this feature (Khulnasoft
  4.2.3 or higher).
...
* Controls whether Khulnasoft replicates a particular type of *.conf file, along
...
* Controls Khulnasoft's search-time conf replication from search heads to search
  nodes.
...
* Conversely, files which are not matched by any whitelist will not be
  replicated.
* Only files located under $KHULNASOFT_HOME/etc will ever be replicated in this
  way.
  * The regex will be matched against the filename, relative to $KHULNASOFT_HOME/etc.
    Example: for a file "$KHULNASOFT_HOME/etc/apps/fancy_app/default/inputs.conf"
...
* The 'name' element is generally just descriptive, with one exception:
  ...
  replicationSettings:refineConf stanza.
* The whitelist_pattern is the Khulnasoft-style pattern matching, which is
...
# mountpoint to access the search head bundles ($KHULNASOFT_HOME/etc).
#******************************************************************************

[searchhead:<searchhead-khulnasoft-server-name>]
* <searchhead-khulnasoft-server-name> is the name of the related searchhead
  installation.
...
* The path to where the search head's bundles are mounted. This must be the
  ...
  a directory that is equivalent to $KHULNASOFT_HOME/etc/. It must contain at least
...
[distributedSearch:<khulnasoft-server-group-name>]
* <khulnasoft-server-group-name> is the name of the khulnasoft-server-group that is
