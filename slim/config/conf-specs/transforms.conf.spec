...
# documentation (http://docs.khulnasoft.com/Documentation)
#
# There is a transforms.conf file in $KHULNASOFT_HOME/etc/system/default/. To
# set custom configurations, place a transforms.conf
# $KHULNASOFT_HOME/etc/system/local/. For examples, see the
# transforms.conf.example file.
#
# You can enable configurations changes made to transforms.conf by typing
# the following search string in Khulnasoft Web:
#
# | extract reload=t
#
...
* The new sourcetype MUST differ from the the original sourcetype.  If the
  ...
  Khulnasoft will make a best effort to log warnings to khulnasoftd.log, but this
...
DEST_KEY = <KEY>
* NOTE: This attribute is only valid for index-time field extractions.
* Specifies where Khulnasoft stores the expanded FORMAT results in accordance
...
* For index-time extractions, DEST_KEY can be set to a number of values
  ...
  * If DEST_KEY = _meta (not recommended) you should also add $0 to the
    ...
    Khulnasoft performs the REGEX (in other words, _meta).
...
* NOTE: This attribute is only valid for index-time field extractions.
* Optional. Khulnasoft writes the DEFAULT_VALUE to DEST_KEY if the REGEX fails.
* Defaults to empty.
...
* NOTE: This attribute is valid for both index-time and search-time field
  extractions.
* Optional. Defines the KEY that Khulnasoft applies the REGEX to.
...
* NOTE: This attribute is only valid for index-time field extractions.
* Optional. When set to true Khulnasoft runs the REGEX multiple times on the
  SOURCE_KEY.
...
* When the event only contains delimiter-separated values (no field names)
  ...
  * Alternately, Khulnasoft reads even tokens as field names and odd tokens as
    field values.
* Khulnasoft consumes consecutive delimiter characters unless you specify a list
  of field names.
...
* NOTE: This attribute is only valid for search-time field extractions.
* Optional. Controls whether Khulnasoft "cleans" the keys (field names) it
...
* NOTE: This attribute is only valid for search-time field extractions.
* Optional. Controls whether Khulnasoft keeps field/value pairs when the value
  is an empty string.
* This option does not apply to field/value pairs that are generated by
  Khulnasoft's autokv extraction. Autokv ignores field/value pairs with empty
  values.
* Defaults to false.

CAN_OPTIMIZE = [true|false]
* NOTE: This attribute is only valid for search-time field extractions.
* Optional. Controls whether Khulnasoft can optimize this extraction out
...
* You might use this if you're running searches under a Search Mode setting
  that disables field discovery--it ensures that Khulnasoft *always* discovers
  specific fields.
* Khulnasoft only disables an extraction if it can determine that none of the
...
* File should be in $KHULNASOFT_HOME/etc/<app_name>/lookups/ for some <app_name>, or in
  $KHULNASOFT_HOME/etc/system/lookups/
...
* Collection should be defined in $KHULNASOFT_HOME/etc/<app_name>/collections.conf
  for some <app_name>
...
* If the lookup is non-temporal (not time-bounded, meaning the time_field
  attribute is not specified), Khulnasoft uses the first <integer> entries, in
  file order.
* If the lookup is temporal, Khulnasoft uses the first <integer> entries in
...
* Default = 0 for both temporal and non-temporal lookups, which means that
  Khulnasoft outputs nothing if it cannot find any matches.
* However, if min_matches > 0, and Khulnasoft get less than min_matches, then
  Khulnasoft provides the default_match value provided (see below).

default_match = <string>
* If min_matches > 0 and Khulnasoft has less than min_matches for any given
...
* The first argument is expected to be a python script (or executable file)
  located in $KHULNASOFT_HOME/etc/<app_name>/bin (or ../etc/searchscripts).
...
* For temporal lookups this specifies the 'strptime' format of the timestamp
  field.
* You can include subseconds but Khulnasoft will ignore them.
...
* NOTE: Any KEY (field name) prefixed by '_' is not indexed by Khulnasoft, in general.
...
* Modifies Khulnasoft's list of key names it considers valid when automatically
  checking your transforms for use of undocumented SOURCE_KEY or DEST_KEY
  values in index-time transformations.
* By adding entries to [accepted_keys], you can tell Khulnasoft that a key that
