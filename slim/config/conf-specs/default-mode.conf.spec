...
# default-mode.conf is a file that exists primarily for Khulnasoft Support and
# Services to configure khulnasoft.

# CAVEATS:

# DO NOT make changes to default-mode.conf without coordinating with Khulnasoft
# Support or Services.  End-user changes to default-mode.conf are not
# supported.
#
# default-mode.conf *will* be removed in a future version of Khulnasoft, along
...
# provide a way to describe the alternate setups used by the Khulnasoft Light
# Forwarder and Khulnasoft Universal Forwarder.
...
[pipeline:<string>]
* Refers to a particular Khulnasoftd pipeline.
* The set of named pipelines is a khulnasoft-internal design.  That does not
  mean that the Khulnasoft design is a secret, but it means it is not external
  for the purposes of configuration.
* Useful information on the data processing system of khulnasoft can be found
  in the external documentation, for example
  http://docs.khulnasoft.com/Documentation/Khulnasoft/latest/Deploy/Datapipeline
...
* The set of named procoessors is again a khulnasoft-internal design component.
