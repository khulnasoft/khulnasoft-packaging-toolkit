...
#
# There is an authentication.conf in $KHULNASOFT_HOME/etc/system/default/.  To
# set custom configurations, place an authentication.conf in
# $KHULNASOFT_HOME/etc/system/local/. For examples, see
# authentication.conf.example.  You must restart Khulnasoft to enable
# configurations.
#
...
authType = [Khulnasoft|LDAP|Scripted|SAML|ProxySSO]
* Specify which authentication system to use.
* Supported values: Khulnasoft, LDAP, Scripted, SAML, ProxySSO.
...
* For LDAP, specify the LDAP strategy name(s) here. If you want Khulnasoft to
...
* For the default "Khulnasoft" authType, this controls how hashed passwords are
  stored in the $KHULNASOFT_HOME/etc/passwd file.
* "MD5-crypt" is an algorithm originally developed for FreeBSD in the early
  ...
  also used by Khulnasoft up through the 5.0.x releases.  MD5-crypt runs the
...
* These SHA-based algorithm can optionally be followed by a number of rounds
  ...
  default value of 5000), khulnasoftd may become unresponsive and connections to
  khulnasoftd (from khulnasoftweb or CLI) will time out.
...
* A valid Multifactor vendor string will enable Multifactor authentication
  and loads support for the corresponding vendor if supported by Khulnasoft.
* Empty string will disable Multifactor authentication in khulnasoft.
* Currently khulnasoft supports duo as a Multifactor authentication vendor.
...
* Be sure that your Khulnasoft server can resolve the host name.

SSLEnabled = [0|1]
* OPTIONAL
* Defaults to disabled (0)
* See the file $KHULNASOFT_HOME/etc/openldap/openldap.conf for SSL LDAP settings

port = <integer>
* OPTIONAL
* This is the port that Khulnasoft should use to connect to your LDAP server.
...
* NOTE: This attribute should use case insensitive matching for its values,
  and the values should not contain whitespace
    * Usernames are case insensitive in Khulnasoft
...
* Set this if groups are mapped using a different attribute
  ...
  * A typical attribute used to map users to groups is 'uid'
    * For example, assume a group declares that one of its members is
      'khulnasoftuser'
    * This implies that every user with 'uid' value 'khulnasoftuser' will be
...
* OPTIONAL
* The LDAP search filter Khulnasoft uses when searching for static groups
...
* OPTIONAL
* The LDAP search filter Khulnasoft uses when searching for dynamic groups
...
* For example, consider the groupMappingAttribute example above using
  ...
  * To declare 'khulnasoftuser' as a group member, its attribute 'member' must
    have the value 'khulnasoftuser'

nestedGroups = <bool>
* OPTIONAL
* Controls whether Khulnasoft will expand nested groups using the
  'memberof' extension.
...
* The mapping of Khulnasoft roles to LDAP groups for the LDAP strategy specified
  by <authSettings-key>
...
* Maps a Khulnasoft role (from authorize.conf) to LDAP groups
* This LDAP group list is semicolon delimited (no spaces).
* List several of these attribute value pairs to map several Khulnasoft roles to
  LDAP Groups
...
* This is the full path to the script, including the path to the program
  that runs it (python)
* For example: "$KHULNASOFT_HOME/bin/python" "$KHULNASOFT_HOME/etc/system/bin/$MY_SCRIPT"
* Note: If a path contains spaces, it must be quoted. The example above
  handles the case where KHULNASOFT_HOME contains a space
...
[cacheTiming]
* Use these settings to adjust how long Khulnasoft will use the answers returned
...
#####################
# Settings for Khulnasoft Authentication mode
#####################

[khulnasoft_auth]
* Settings for Khulnasoft's internal authentication system.
...
fqdn = <string>
* OPTIONAL
* The fully qualified domain name where this khulnasoft instance is running.
* If this value is not specified, Khulnasoft will default to the value specified
  in server.conf.
* If this value is specified and 'http://' or 'https://' prefix is not
  present, khulnasoft will use the ssl setting for khulnasoftweb.
* Khulnasoft will use this information to populate the 'assertionConsumerServiceUrl'.
...
* If internal port redirection is needed, set this port and the
  ...
  instead of the khulnasoftweb port.
...
* When this setting is absent, Khulnasoft will cache the role information from the saml
  assertion and use it to run saved searches.

idpCertPath = <Pathname>
* OPTIONAL
* This setting is required if 'signedAssertion' is set to true.
* This value is relative to $KHULNASOFT_HOME/etc/auth/idpCerts.
* The value for this setting can be the name of the certificate file or a directory.
* If it is empty, Khulnasoft will automatically verify with certificates in all subdirectories
  present in $KHULNASOFT_HOME/etc/auth/idpCerts.
...
* OPTIONAL
* This tells Khulnasoft whether to sign AuthNRequests.
* Defaults to true.

signedAssertion = [true|false]
* OPTIONAL
* This tells Khulnasoft if the SAML assertion has been signed by the IDP
* If set to false, Khulnasoft will not verify the signature of the assertion
...
* This string will obfuscated upon khulnasoftd startup.
...
* If set to false, Khulnasoft will not verify the signature in the response
  using the certificate of the IDP.
* Defaults to true.

redirectAfterLogoutToUrl = <url>
* OPTIONAL
* The user will be redirected to this url after logging out of Khulnasoft.
* If this is not specified and a idpSLO is also missing, the user will be
  redirected to khulnasoft.com after logout.

defaultRoleIfMissing = <khulnasoft role>
* OPTIONAL
* If the IDP does not return any AD groups or khulnasoft roles as a part of the
...
* OPTIONAL
* Determines the time for which Khulnasoft will cache the user and role
  information.
* Once the ttl expires, Khulnasoft will make an attribute query request to
...
* If not set, Khulnasoft uses the setting specified in server.conf.

sslAltNameToCheck = <alternateName1>, <alternateName2>, ...
* OPTIONAL
* If this value is set, and 'sslVerifyServerCert' is set to true,
  khulnasoftd will also be willing to verify certificates which have a so-called
...
* Certificates are auto-generated upon first starting Khulnasoft.
* You may replace the auto-generated certificate with your own.
* Default is $KHULNASOFT_HOME/etc/auth/server.pem.
* If not set, Khulnasoft uses the setting specified in
...
* If unset, Khulnasoft uses the setting specified in server.conf.
* Default is password.
...
* OPTIONAL
* Comma separated list of khulnasoft roles that should be blacklisted
  from being auto-mapped by khulnasoft from the IDP Response.

blacklistedUsers = <comma separated list of user names>
* OPTIONAL
* Comma separated list of user names from the IDP response to be
  blacklisted by khulnasoft platform.
...
* The mapping of Khulnasoft roles to SAML groups for the SAML stanza specified
  by <authSettings-key>
* If a SAML group is not explicitly mapped to a Khulnasoft role, but has
  same name as a valid Khulnasoft role then for ease of configuration, it is
  auto-mapped to that Khulnasoft role.
* Follow this stanza name with several Role-to-Group(s) mappings as defined
  below.

<Khulnasoft RoleName> = <SAML group string>
* Maps a Khulnasoft role (from authorize.conf) to SAML groups
* This SAML group list is semicolon delimited (no spaces).
* List several of these attribute value pairs to map several Khulnasoft roles to
  SAML Groups.
* If role mapping is not specified, Khulnasoft expects Khulnasoft roles in the
...
* The mapping of SAML user to Khulnasoft roles for the SAML stanza specified
  by <authSettings-key>
...
<SAML User> = <Khulnasoft Roles string>
* Maps a SAML user to Khulnasoft role (from authorize.conf)
* This Khulnasoft Role list is semicolon delimited (no spaces).
...
* Khulnasoft expects email, real name and roles to be returned as SAML
  Attributes in SAML assertion. This stanza can be used to map attribute names
  to what Khulnasoft expects. These are optional settings and are only needed for
  certain IDPs.
...
* The mapping of Khulnasoft roles to groups passed in headers from proxy server.
* If a group is not explicitly mapped to a Khulnasoft role, but has
  same name as a valid Khulnasoft role then for ease of configuration, it is
  auto-mapped to that Khulnasoft role.
* Follow this stanza name with several Role-to-Group(s) mappings as defined
  below.

<Khulnasoft RoleName> = <Group string>
* Maps a Khulnasoft role (from authorize.conf) to groups
...
* The mapping of ProxySSO user to Khulnasoft roles
* Follow this stanza name with several User-to-Role(s) mappings as defined
  below.

<ProxySSO User> = <Khulnasoft Roles string>
* Maps a ProxySSO user to Khulnasoft role (from authorize.conf)
* This Khulnasoft Role list is semicolon delimited (no spaces).
...
defaultRoleIfMissing = <khulnasoft role>
* OPTIONAL
* If khulnasoft roles cannot be determined based on role mapping, use default configured
* khulnasoft role.

blacklistedAutoMappedRoles = <comma separated list of roles>
* OPTIONAL
* Comma separated list of khulnasoft roles that should be blacklisted
  from being auto-mapped by khulnasoft from the proxy server headers.
...
* <filename> should be the name of a Python script located in one of the
  following directories:
    $KHULNASOFT_HOME/etc/apps/*/bin
    $KHULNASOFT_HOME/etc/system/bin
    $KHULNASOFT_HOME/etc/searchscripts
...
* If multiple Khulnasoft instances must store separate sets of secrets within the
...
* REQUIRED
* Duo's integration key for khulnasoft. Must be of size = 20.
* Integration key will be obfuscated before being saved here for security.

secretKey = <string>
* REQUIRED
* Duo's secret key for khulnasoft. Must be of size = 40.
* Secret key will be obfuscated before being saved here for security.

appSecretKey = <string>
* REQUIRED
* Khulnasoft application specific secret key which should be random and locally generated.
...
* If set to true, Khulnasoft will bypass Duo Multifactor Authentication when the service is
  unavailable.

timeout = <int>
* OPTIONAL
* It determines the connection timeout in seconds for the outbound duo HTTPS connection.
* If not set, Khulnasoft will use its default HTTPS connection timeout which is 12 seconds.

sslVersions = <versions_list>
* OPTIONAL
* Comma-separated list of SSL versions to support for incoming connections.
* The versions available are "ssl3", "tls1.0", "tls1.1", and "tls1.2".
* If not set, Khulnasoft uses the sslVersions provided in server.conf

cipherSuite = <cipher suite string>
* OPTIONAL
* If set, Khulnasoft uses the specified cipher string for the HTTP server.
* If not set, Khulnasoft uses the cipher string provided in server.conf

ecdhCurves = <comma separated list of ec curves>
* OPTIONAL
* ECDH curves to use for ECDH key negotiation.
* If not set, Khulnasoft uses the ecdh curve names provided in server.conf
...
* OPTIONAL
* Not set by default.
* If this value is set, Khulnasoft will limit outbound duo HTTPS connections
