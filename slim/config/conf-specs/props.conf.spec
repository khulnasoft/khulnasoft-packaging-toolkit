#   Version 20170104
#
# This file contains possible attribute/value pairs for configuring Khulnasoft's
...
# * Overriding Khulnasoft's automated host and source type matching. You can use
#   props.conf to:
...
# configuration.
#
# You can find more information on these topics by searching the Khulnasoft
# documentation (http://docs.khulnasoft.com/Documentation/Khulnasoft).
#
# There is a props.conf in $KHULNASOFT_HOME/etc/system/default/.  To set custom
# configurations, place a props.conf in $KHULNASOFT_HOME/etc/system/local/. For
# help, see props.conf.example.
#
# You can enable configurations changes made to props.conf by typing the
# following search string in Khulnasoft Web:
#
# | extract reload=T
#
...
# distributed Khulnasoft deployments, see the Distributed Deployment Manual.
...
http://docs.khulnasoft.com/Documentation/Khulnasoft/latest/Data/Specifyinputpathswithwildcards
...
settings, Khulnasoft applies all of these settings.

However, suppose two [<spec>] stanzas supply the same setting. In this case,
Khulnasoft chooses the value to apply based on the ASCII order of the patterns
in question.
...
CHARSET = <string>
* When set, Khulnasoft assumes the input from the given [<spec>] is in the
  specified encoding.
...
* If the source encoding is valid, but some characters from the [<spec>] are
  ...
  hex (for example, "\xF3").
* When set to "AUTO", Khulnasoft attempts to automatically determine the character encoding and
  convert text from that encoding to UTF-8.
* For a complete list of the character sets Khulnasoft automatically detects,
  see the online documentation.
* This setting applies at input time, when data is first read by Khulnasoft.
  The setting is used on a Khulnasoft system that has configured inputs
  acquiring the data.
* Defaults to ASCII.
...
* Wherever the regex matches, Khulnasoft considers the start of the first
...
* The contents of the first capturing group are discarded, and will not be
  present in any event.  You are telling Khulnasoft that this text comes between
  lines.
* NOTE: You get a significant boost to processing speed when you use
  ...
  * Using LINE_BREAKER to delimit events is discussed in more detail in the web
    ...
    http://docs.khulnasoft.com/Documentation/Khulnasoft/latest/Data/indexmulti-lineevents
...
* When there is leftover data from a previous raw chunk,
  ...
  the raw chunk (with the next chunk concatenated) that Khulnasoft applies the
...
* When set to true, Khulnasoft combines several lines of data into a single
...
# define how Khulnasoft builds multiline events.

BREAK_ONLY_BEFORE_DATE = [true|false]
* When set to true, Khulnasoft creates a new event only if it encounters a new
  line with a date.
...
* When set, Khulnasoft creates a new event only if it encounters a new line that
  matches the regular expression.
* Defaults to empty.

MUST_BREAK_AFTER = <regular expression>
* When set and the regular expression matches the current line, Khulnasoft
  creates a new event for the next input line.
* Khulnasoft may still break before the current line if another rule matches.
* Defaults to empty.

MUST_NOT_BREAK_AFTER = <regular expression>
* When set and the current line matches the regular expression, Khulnasoft does
...
* Khulnasoft breaks after the specified number of lines are read.
* Defaults to 256 (lines).

# Use the following attributes to handle better load balancing from UF.
# Please note the EVENT_BREAKER properties are applicable for Khulnasoft Universal
# Forwarder instances only.

EVENT_BREAKER_ENABLE = [true|false]
* When set to true, Khulnasoft will split incoming data with a light-weight
...
* When set, Khulnasoft will use the setting to define an event boundary at the
...
DATETIME_CONFIG = <filename relative to $KHULNASOFT_HOME>
...
* This configuration may also be set to "NONE" to prevent the timestamp
  ...
  * "NONE" will leave the event time set to whatever time was selected by
    the input layer
    * For data sent by khulnasoft forwarders over the khulnasoft protocol, the input
...
* Defaults to /etc/datetime.xml (for example, $KHULNASOFT_HOME/etc/datetime.xml).

TIME_PREFIX = <regular expression>
* If set, khulnasoft scans the event text for a match for this regex in event
...
* Specifies how far (in characters) into an event Khulnasoft should look for a
  timestamp.
...
* Otherwise, use the timezone of the system that is running khulnasoftd.
* Defaults to empty.

TZ_ALIAS = <key=value>[,<key=value>]...
* Provides khulnasoft admin-level control over how timezone strings extracted
...
* There is no requirement to use TZ_ALIAS if the traditional Khulnasoft default
...
* Specifies the maximum number of days in the past, from the current date as
  ...
  that an extracted date can be valid. Khulnasoft still indexes events with dates
...
* For example, if MAX_DAYS_AGO = 10, Khulnasoft applies the timestamp of the last
  acceptable event to events with extracted timestamps older than 10 days in the
  past. If no acceptable event exists, Khulnasoft applies the current timestamp.
...
* For example, if MAX_DAYS_HENCE = 3, Khulnasoft applies the timestamp of the last
  acceptable event to events with extracted timestamps more than 3 days in the
  future. If no acceptable event exists, Khulnasoft applies the current timestamp.
...
* This setting prevents Khulnasoft Enterprise from rejecting events with timestamps
  that are out of order.
* Do not use this setting to filter events because Khulnasoft Enterprise uses
  complicated heuristics for time parsing.
* Khulnasoft Enterprise warns you if an event timestamp is more than <integer>
...
* After Khulnasoft Enterprise throws the warning, it only rejects an event if it
  cannot apply a timestamp to the event (for example, if Khulnasoft cannot
...
* This feature and all of its settings apply at input time, when data is
  first read by Khulnasoft.  The setting is used on a Khulnasoft system that has
...
* Tells Khulnasoft the type of file and the extraction and/or parsing method
  Khulnasoft should use on the file.
...
* Some files contain preamble lines. This attribute specifies a regular
  expression which allows Khulnasoft to ignore these preamble lines, based on
...
* Tells Khulnasoft the line number of the line within the file that contains the
  header fields.  If set to 0, Khulnasoft attempts to locate the header fields
  within the file automatically.
* The default value is set to 0.

FIELD_DELIMITER = <character>
* Tells Khulnasoft which character delimits or separates fields in the specified
  file or source.
* This attribute supports the use of the special characters described above.

HEADER_FIELD_DELIMITER = <character>
* Tells Khulnasoft which character delimits or separates header fields in the
  specified file or source.
* This attribute supports the use of the special characters described above.

FIELD_QUOTE = <character>
* Tells Khulnasoft the character to use for quotes in the specified file or
  source.
* This attribute supports the use of the special characters described above.

HEADER_FIELD_QUOTE = <character>
* Specifies Khulnasoft the character to use for quotes in the header of the
  specified file or source.
* This attribute supports the use of the special characters described above.

TIMESTAMP_FIELDS = [ <string>,..., <string>]
* Some CSV and structured files have their timestamp encompass multiple
  fields in the event separated by delimiters. This attribue tells Khulnasoft to
...
* Tells Khulnasoft the placeholder to use in events where no value is present.
...
NOTE: Index-time field extractions have performance implications. Creating
      additions to Khulnasoft's default set of indexed fields is ONLY
...
company_id to the list of fields extracted by Khulnasoft at index time. This is
because at search time, Khulnasoft will want to check each instance of the value
1 to see if it matches company_id, and that kind of thing slows down
performance when you have Khulnasoft searching a large set of data.
...
extracted by Khulnasoft at index time.
...
expression that Khulnasoft uses to extract fields at search time. You can use
...
* Configure extractions for multivalued fields. You can have Khulnasoft append
...
* Extract fields with names beginning with numbers or underscores.
  Ordinarily, Khulnasoft's key cleaning functionality removes leading numeric
...
* For each field extraction, Khulnasoft takes the configuration from the highest
...
* Use a comma-separated list to apply multiple transform stanzas to a single
  TRANSFORMS extraction. Khulnasoft applies them in the list order. For example,
...
* Use a comma-separated list to apply multiple transform stanzas to a single
  REPORT extraction.
  Khulnasoft applies them in the list order. For example, this sequence insures
...
* If your regex needs to end with 'in <string>' where <string> is *not* a
  ...
  Khulnasoft doesn't try to match <string> to a field name.
...
* The trimming logic applies only to space characters, not tabs, or other
  whitespace.
* NOTE: The Khulnasoft UI currently has limitations with displaying and
  ...
  * The Khulnasoft search language and included commands will respect the spaces.
* Defaults to true.
...
* If the file has a list of columns and each event contains a field value
  (without field name), Khulnasoft picks a suitable header line to use to for
  extracting field names.
* If the file has a list of columns and each event contains a field value
  (without a field name), Khulnasoft picks a suitable header line to use for
  field extraction.
...
* Used to specify a sed script which Khulnasoft applies to the _raw field.
...
* Khulnasoft processes calculated fields after field extraction and field
...
* <match_field> specifies a field in the lookup table to match on.
  * By default Khulnasoft looks for a field with that same name in the event to
...
* <output_field> specifies a field in the lookup entry to copy into each
  ...
  * If you do not specify an <output_field_in_event> value, Khulnasoft
    uses <output_field>.
...
* Khulnasoft processes lookups after it processes field extractions, field
...
* When set to true, Khulnasoft processes binary files.
...
* When enabled, Khulnasoft will try to avoid reading in null bytes at the end of
  a file.
* When false, khulnasoft will assume that all the bytes in the file should be
  read and indexed.
...
* Subtleties of 'true' vs 'auto':
  * 'true' is the khulnasoft-on-windows historical behavior of trimming all null
           bytes.
...
* Specifies that Khulnasoft Web should use the specific segmenter (from
...
* Do not change the set of default <segment selection> choices, unless you
  ...
  <segment selection> choices to appear in Khulnasoft Web, you will need to edit
  the Khulnasoft Web UI.
...
* Set CHECK_METHOD endpoint_md5 to have Khulnasoft checksum of the first and
  last 256 bytes of a file. When it finds matches, Khulnasoft lists the file as
...
* Set CHECK_METHOD = modtime to check only the modification time of the
  file.
* Settings other than endpoint_md5 cause Khulnasoft to index the entire file for
...
* PREFIX_SOURCETYPE = true sets the source type to "<sourcename>-too_small",
  ...
  * The advantage of PREFIX_SOURCETYPE = true is that not all small files
    ...
    effective.
  * For example, a Khulnasoft search of "sourcetype=access*" will retrieve
...
* sourcetype assignment settings on a system receiving forwarded khulnasoft data
...
* Set to any other string to throw an error in the khulnasoftd.log if you are
  running Khulnasoftlogger in debug mode.
* This setting applies at input time, when data is first read by Khulnasoft.
  The setting is used on a Khulnasoft system that has configured inputs
  acquiring the data.
* Defaults to empty.
...
* Must be a shell command that takes input on stdin and produces output on
  stdout.
* Use _auto for Khulnasoft's automatic handling of archive files (tar, tar.gz,
  tgz, tbz, tbz2, zip)
* This setting applies at input time, when data is first read by Khulnasoft.
  The setting is used on a Khulnasoft system that has configured inputs
  acquiring the data.
...
* If this field is empty (for a matching archive file props lookup) Khulnasoft
...
* The larger the maxDist value, the more forgiving Khulnasoft will be with
  differences.
...
* These settingg apply at input time, when data is first read by Khulnasoft,
  such as on a forwarder.
...
* Determines whether to use the inline ***KHULNASOFT*** directive to rewrite index-time fields.
  * If "always", any line with ***KHULNASOFT*** can be used to rewrite
    index-time fields.
  * If "firstline", only the first line can be used to rewrite
    index-time fields.
  * If "none", the string ***KHULNASOFT*** is treated as normal data.
