...
# inputs.conf.
#
# There is an inputs.conf in $KHULNASOFT_HOME/etc/system/default/.  To set custom
# configurations, place an inputs.conf in $KHULNASOFT_HOME/etc/system/local/.  For
# examples, see inputs.conf.example. You must restart Khulnasoft to enable new
# configurations.
#
...
* If set to '$decideOnStartup', will be interpreted as hostname of executing
  machine; this will occur on each khulnasoftd startup.
...
* The literal default conf value is $decideOnStartup, but at installation
  ...
  $KHULNASOFT_HOME/etc/system/local/inputs.conf default stanza, which is the
...
* If unset, Khulnasoft picks a source type based on various aspects of the data.
...
* Set queue to "parsingQueue" to apply props.conf and other parsing rules to
  ...
  http://docs.khulnasoft.com/Documentation.
...
* To forward data from the "_internal" index, _TCP_ROUTING must explicitly be
  set to either "*" or a specific khulnasofttcp target group.
...
* CAUTION: Improper use of this setting will cause data to be re-indexed.  You
  might want to consult with Khulnasoft Support before adjusting this value - the
...
* Files placed on the ignore list will not be checked again for any
  reason until the Khulnasoft software restarts, or the file monitoring subsystem
...
* Treat this setting as an 'action':
  * Enable this setting and start the Khulnasoft software.
...
#****************************************
# BATCH  ("Upload a file" in Khulnasoft Web):
#****************************************
...
* "dns" sets the host to the reverse DNS entry for the IP address of the system
  sending the data.
* "none" leaves the host as specified in inputs.conf, typically the khulnasoft
  system hostname.
* Defaults to "dns".
...
* Each rule can be in one of the following formats:
    ...
    3. A DNS name, possibly with a '*' used as a wildcard
       (examples: "myhost.example.com", "*.khulnasoft.com")
...
# Global settings for khulnasofttcp. Used on the receiving side for data forwarded
# from a forwarder.
...
* This specifies the global keepalive setting for all khulnasofttcp ports.
...
* Specifies the time, in seconds, to wait before closing the khulnasofttcp port.
* If the receiver is unable to insert received data into the configured queue
  for more than the specified number of seconds, it closes the khulnasofttcp port.
...
# Forwarder-specific settings for khulnasofttcp.

[khulnasofttcp://[<remote server>]:<port>]
* Receivers use this input stanza.
* This is the same as the [tcp://] stanza, except the remote server is assumed
  to be a Khulnasoft instance, most likely a forwarder.
...
connection_host = [ip|dns|none]
* For khulnasofttcp, the host or connection_host will be used if the remote Khulnasoft
...
enableS2SHeartbeat = [true|false]
* This specifies the keepalive setting for the khulnasofttcp port.
* This option is used to detect forwarders which might have become unavailable
  due to network, firewall, or other problems.
* The receiver monitors the connection for presence of heartbeat, and if it
  ...
  connection.
* This overrides the default value specified at the global [khulnasofttcp] stanza.
* Defaults to true (heartbeat monitoring enabled).
...
negotiateNewProtocol = [true|false]
* See the description for [khulnasofttcp].

concurrentChannelLimit = <unsigned integer>
* See the description for [khulnasofttcp].

[khulnasofttcp:<port>]
* This input stanza is the same as [khulnasofttcp://[<remote server>]:<port>], but
  does not have a remote server restriction.
* Please see documentation for [khulnasofttcp://[<remote server>]:<port>] for
...
* The 'compressed' setting is used for non-SSL connections. However, if you
  ...
  the same as on the forwarder, as khulnasofttcp protocol expects the same
...
compressed = [true|false]
* See comments for [khulnasofttcp:<port>].
...
negotiateNewProtocol = [true|false]
* See comments for [khulnasofttcp].
...
* Upon first use, the input encrypts and rewrites the password to
  $KHULNASOFT_HOME/etc/system/local/inputs.conf.
...
* Khulnasoft only supports named curves that have been specified by their SHORT name.
...
* Khulnasoft only supports named curves that have been specified by their SHORT names.
...
* If there is no match, assume that the Khulnasoft instance is not authenticated
  against this server.
...
* Similar to the [tcp://] stanza, except that this stanza causes the Khulnasoft
...
* The <cmd> must reside in one of the following directories:
  ...
  * $KHULNASOFT_HOME/etc/apps/$YOUR_APP/bin/
  * $KHULNASOFT_HOME/bin/scripts/
* The path to <cmd> can be an absolute path, make use of an environment
  variable such as $KHULNASOFT_HOME, or use the special pattern of an initial '.'
...
* You might want to explicitly set the setting to false for scripts
  ...
  a Khulnasoft best practice.

#*******
# File system change monitor (fschange monitor)
#*******
#
# The file system change monitor has been deprecated as of Khulnasoft Enterprise
...
* You must copy the full contents of the khulnasoft_httpinput app directory
  to this directory for the configuration to work.
* When disabled, the input writes its configuration to
  $KHULNASOFT_HOME/etc/apps by default.
...
* HEC shares SSL settings with the Khulnasoft management server and cannot have
  'enableSSL' set to true when the Khulnasoft management server has SSL disabled.
...
* The Khulnasoft software automatically generates certificates when it first
  starts.
...
* Defaults to $KHULNASOFT_HOME/etc/auth.
...
* If you set this setting and also set 'requireClientCert' to true,
  khulnasoftd limits most inbound HTTPS connections to hosts that use
...
sslAltNameToCheck = <alternateName1>, <alternateName2>, ...
* If you set this setting and also set 'requireClientCert' to true,
  khulnasoftd can verify certificates that have a so-called
...
* This can help avoid a client being tricked later by a Man-In-The-Middle
  ...
  example, if Khulnasoft Web is in default non-SSL mode this can break the
...
# ***********
# Khulnasoft on Windows ships with several Windows-only inputs. They are
...
* Each perfmon:// stanza represents an individually configured performance
  monitoring input. If you configure the input through Khulnasoft Web, then the
  value of "<NAME>" matches what was specified there. While you can add
  performance monitor inputs manually, Khulnasoft recommends that you use Khulnasoft
...
* Each WinEventLog:// stanza represents an individually configured WinEventLog
  monitoring input. If you you configure the input through Khulnasoft Web, the
  value of "<NAME>" matches what was specified there. While you can add
  event log monitor inputs manually, Khulnasoft recommends that you use Khulnasoft
...
* This is an advanced setting. Contact Khulnasoft Support before you change it.
...
* If troubleshooting identifies that the Event Log input is a bottleneck in
  ...
  * NOTE: Khulnasoft Support has seen cases where large values can result in a
...
# processing load in network transfer and computation on the Khulnasoft
...
* For a detailed definition of these keys, see the online documentation:
  http://docs.khulnasoft.com/Documentation/Khulnasoft/latest/Data/MonitorWindowsdata#Create_advanced_filters_with_.27whitelist.27_and_.27blacklist.27
...
* Each admon:// stanza represents an individually configured Active
  Directory monitoring input. If you configure the input with Khulnasoft Web,
  then the value of "<NAME>" matches what was specified there. While
  you can add Active Directory monitor inputs manually, Khulnasoft recommends
  that you use Khulnasoft Web to configure Active Directory monitor
...
* The user that you configure the Khulnasoft software to run as at
...
* If you configure the inputs with Khulnasoft Web, the value of "<NAME>" matches
  what was specified there. While you can add event log monitor inputs
  manually, recommends that you use Khulnasoft Web to configure
...
* The "<name>" component of the stanza name will be used as the source field
  ...
  that).
* If you configure the input in Khulnasoft web, the value of "<name>" matches
...
* Each WinPrintMon:// stanza represents an WinPrintMon monitoring input.
  The value of "<name>" matches what was specified in Khulnasoft Web.
...
* Each WinNetMon:// stanza represents an individually configured network
  ...
  in Khulnasoft Web. Khulnasoft recommends that you use Khulnasoft Web to
