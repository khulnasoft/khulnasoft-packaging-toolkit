...
# There is an authorize.conf in $KHULNASOFT_HOME/etc/system/default/.  To set
# custom configurations, place an authorize.conf in
# $KHULNASOFT_HOME/etc/system/local/. For examples, see authorize.conf.example.
...
* DO NOT edit, remove, or add capability stanzas. The existing capabilities
  are the full set of Khulnasoft system capabilities.
* Khulnasoft adds all of its capabilities this way
...
### Descriptions of Khulnasoft system capabilities
...
* Required to add and edit inputs for monitoring Khulnasoft's Active Directory.
...
[capability::edit_khulnasofttcp]
* Required to change settings for receiving TCP input from another Khulnasoft
  instance.

[capability::edit_khulnasofttcp_ssl]
* Required to list or edit any SSL specific settings for Khulnasoft TCP input.

[capability::edit_khulnasofttcp_token]
* Required to list or edit khulnasofttcptokens which can be used on a receiving
...
* Note that Khulnasoft users may edit certain aspects of their information
...
* Used for distributing search to old 4.0.x Khulnasoft instances.
...
[capability::restart_khulnasoftd]
* Required to restart Khulnasoft through the server control handler.
...
* Required to reload CRL information within Khulnasoft
