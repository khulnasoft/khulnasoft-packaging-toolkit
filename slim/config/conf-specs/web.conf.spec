...
# Khulnasoft's web interface.
#
# There is a web.conf in $KHULNASOFT_HOME/etc/system/default/.  To set custom
# configurations, place a web.conf in $KHULNASOFT_HOME/etc/system/local/.  For
# examples, see web.conf.example.  You must restart Khulnasoft to enable
# configurations.
#
...
[settings]
* Set general KhulnasoftWeb configuration options under this stanza name.
...
startwebserver = [0 | 1]
* Set whether or not to start KhulnasoftWeb.
* 0 disables KhulnasoftWeb, 1 enables it.
* Defaults to 1.

httpport = <port_number>
* Must be present for KhulnasoftWeb to start.
* If omitted or 0 the server will NOT start an http listener.
* If using SSL, set to the HTTPS port number.
* Defaults to 8000.

mgmtHostPort = <IP:port>
* Location of khulnasoftd.
...
* If set to "0", this will prevent the application server from
  being run from khulnasoftd.  Instead, khulnasoftweb will be started as
  a separate python-based service which directly listens to the
  'httpport'.  This is how Khulnasoft 6.1.X and earlier behaved.
* Generally, you should only set one port number here.  For most
  ...
  port numbers here and khulnasoftd will start a load-balanced
...
khulnasoftdConnectionTimeout = <integer>
...
enableKhulnasoftWebClientNetloc = [True | False]
* Control if the khulnasoft web client can override the client network location
* Defaults to False.

enableKhulnasoftWebSSL = [True | False]
...
* The path to the file containing the web server's SSL certificate's private
  key.
* A relative path is interpreted relative to $KHULNASOFT_HOME and may not refer
  outside of $KHULNASOFT_HOME (e.g., no ../somewhere).
* An absolute path can also be specified to an external key.
* See also 'enableKhulnasoftWebSSL' and 'serverCert'.

serverCert = <path>
* Full path to the PEM format Khulnasoft web server certificate file.
...
* Default is $KHULNASOFT_HOME/etc/auth/khulnasoftweb/cert.pem.
* See also 'enableKhulnasoftWebSSL' and 'privKeyPath'.
...
* Requires that any HTTPS client that connects to the khulnasoftweb HTTPS
...
* When true, it is mandatory to configure khulnasoftd with same root CA in server.conf.
  This is needed for internal communication between khulnasoftd and khulnasoftweb.
* Defaults to false.
...
* If this value is set, and 'requireClientCert' is set to true,
  khulnasoftweb will verify certificates which have a so-called
...
serviceFormPostURL = http://docs.khulnasoft.com/Documentation/Khulnasoft
* This attribute is deprecated since 5.0.3

userRegistrationURL = https://www.khulnasoft.com/page/sign_up
updateCheckerBaseURL = http://quickdraw.Khulnasoft.com/js/
docsCheckerBaseURL = http://quickdraw.khulnasoft.com/help
* These are various Khulnasoft.com urls that are configurable.
* Setting updateCheckerBaseURL to 0 will stop the KhulnasoftWeb from pinging
  Khulnasoft.com for new versions of itself.
...
* Relative paths are interpreted as relative to $KHULNASOFT_HOME.
* Relative paths may not refer outside of $KHULNASOFT_HOME (eg. no ../somewhere).
* An absolute path can also be specified to an external key.
* Not set by default.
...
* Default setting is '/'
* Ex: if you want to proxy the khulnasoft UI at http://khulnasoft:8000/khulnasoftui,
  then set root_endpoint = /khulnasoftui
...
* Default is share/khulnasoft/search_mrsparkle/exposed
...
* Defaults to "khulnasoft>" but can be changed to whatever the user would like.
...
* Defaults to share/khulnasoft/search_mrsparkle/templates

module_dir = <relative_filesystem_path>
* Base path to UI module assets
* Defaults to share/khulnasoft/search_mrsparkle/modules
...
* Specifies whether or not to override the MIME type for JSON data served up
  by khulnasoftweb endpoints with content-type="text/plain; charset=UTF-8"
* If True, khulnasoftweb endpoints (other than proxy) that serve JSON data will
  serve as "text/plain; charset=UTF-8"
* If False, khulnasoftweb endpoints that serve JSON data will serve as "application/json; charset=UTF-8"

enable_proxy_write = [True | False]
* Indicates if the /khulnasoftd proxy endpoint allows POST operations
* If True, both GET and POST operations are proxied through to khulnasoftd
* If False, only GET operations are proxied through to khulnasoftd
* Setting this to False will prevent many client-side packages (such as the
  Khulnasoft JavaScript SDK) from working correctly
* Defaults to True
...
* See js/logger.js Khulnasoft.Logger.Mode for mode implementation details and if
...
* When appServerPorts is set to a non-zero value, khulnasoftd can keep a
...
* Specifies whether Integrated PDF Generation is available on this search
  head
* This is used to bypass an extra call to khulnasoftd
* Defaults to 1 on platforms where node is supported, defaults to 0
  otherwise

version_label_format = <printf_string>
* Internal config
* Used to override the version reported by the UI to *.khulnasoft.com resources
* Defaults to: %s

auto_refresh_views = [0 | 1]
* Specifies whether the following actions cause the appserver to ask khulnasoftd
  to reload views from disk.
  * Logging in via the UI
  * Switching apps
  * Clicking the Khulnasoft logo
* Defaults to 0.

#
# Khulnasoft bar options
#
...
* Caution: There is a potential security concern regarding Khulnasoft's
...
* If the browser or other http agent were to specify the value of this
  ...
  it blank.
* However, Khulnasoft (cherrypy) will normalize headers containing the dash,
  and the underscore to the same value.  For example USER-NAME and
  USER_NAME will be treated as the same in KhulnasoftWeb.
* This means that if the browser provides REMOTE-USER and khulnasoft accepts
...
* This value is used by khulnasoft the match against header name.
...
* Value "0" is for compatibility with older versions of Khulnasoft, but
...
* Permissive: Requests to Khulnasoft Web that originate from an untrusted IP
  address are redirected to a login page where they can log into Khulnasoft
  without using SSO.
* Strict: All requests to khulnasoftweb will be restricted to those originating
...
* Khulnasoftweb verifies it is receiving data from the proxy host for all
  SSO requests.
...
* Specifies the root URI path on which to serve khulnasoftweb unit and
  integration testing resources.
* Development only setting
* Defaults to '/testing'

testing_dir = <relative_file_path>
* Specifies the path relative to $KHULNASOFT_HOME that contains the testing
  files to be served at endpoint defined by 'testing_endpoint'.
* Development only setting
* Defaults to 'share/khulnasoft/testing'
...
* closing the connection with khulnasoftd. If you do not set a value for
* export_timeout, the value in khulnasoftdConnectionTimeout is used.
...
* By default, khulnasoftweb will listen for incoming connections using
  IPv4 only
* To enable IPv6 support in khulnasoftweb, set this to "yes".  Khulnasoftweb
  will simultaneously listen for connections on both IPv4 and IPv6
* To disable IPv4 entirely, set this to "only", which will cause khulnasoftweb
...
* Stored in default Khulnasoft /var/log directory
...
* If set to True and Khulnasoftweb is configured to server requests using HTTPS
  (see the enableKhulnasoftWebSSL setting) then the browser will only transmit
...
* Use an absolute path to store sessions outside of the khulnasoft tree
* Defaults to storage_type=file, storage_path=var/run/khulnasoft
...
* Used for running Apache as a proxy for Khulnasoft UI, typically for SSO
...
* For Apache 1.x proxies only. Set this attribute to "true". This
  configuration instructs CherryPy (the Khulnasoft Web HTTP server) to look for
...
* Used for setting the proxy base url in Khulnasoft
* Defaults to an empty value

pid_path = <filepath>
* Specifies the path to the PID file
* Equals precisely and only var/run/khulnasoft/khulnasoftweb.pid
* NOTE: Do not change this parameter.

enabled_decomposers = <intention> [, <intention>]...
* Added in Khulnasoft 4.2 as a short term workaround measure for apps which
...
* Modifies search decomposition, which is a khulnasoft-web internal behavior.
...
* How long, in seconds, that the KhulnasoftWeb HTTP server allows a keep-alive
...
* This can help avoid a client being tricked later by a Man-In-The-Middle
  ...
  For example, if khulnasoftweb is in default non-SSL mode this can break the
...
termsOfServiceDirectory = <directory>
  ...
  * The TOS file can either be a full HTML document or plain text, but it must have the
    ".html" suffix
  * It is not necessary to restart Khulnasoft when adding files to the TOS directory
...
appServerProcessShutdownTimeout = <nonnegative integer>[smhd]
  ...
  * The amount of time khulnasoftd will wait "politely" for a Python-based application server process to handle outstanding/existing requests.
...
* Paths are relative to $KHULNASOFT_HOME.
* Defaults to empty
...
* Customize the favicon image across the entire application. If no favicon image file, the favicon defaults to the Khulnasoft favicon.
  ...
  * Place favicon image file in default or manual location:
    * Default destination folder: $KHULNASOFT_HOME/etc/apps/search/appserver/static/customfavicon.
      * Example: If your favicon image is located at $KHULNASOFT_HOME/etc/apps/search/appserver/static/customfavicon/favicon.ico, type customFavicon = customfavicon/favicon.ico.
    * Manual location: $KHULNASOFT_HOME/etc/apps/<myApp>/appserver/static/<pathToMyFile>, and type customFavicon = <myApp:pathToMyFile>.

loginCustomLogo = <fullUrl, pathToMyFile, myApp:pathToMyFile, or blank for default>
* Customize the logo image on the login page. If no image file, the logo defaults to the Khulnasoft logo.
* Supported images are:
  * Full URL image file (secured or not secured), such as https://www.khulnasoft.com/logo.png or http://www.khulnasoft.com/logo.png.
  * Image file, such as .jpg or .png. All image formats are supported.
    * Place logo image file in default or manual location:
      * Default destination folder: $KHULNASOFT_HOME/etc/apps/search/appserver/static/logincustomlogo.
        * Example: If your logo image is located at $KHULNASOFT_HOME/etc/apps/search/appserver/static/logincustomlogo/logo.png, type loginCustomLogo = logincustomlogo/logo.png.
      * Manual location: $KHULNASOFT_HOME/etc/apps/<myApp>/appserver/static/<pathToMyFile>, and type loginCustomLogo = <myApp:pathToMyFile>.
...
* Defaults to "default".
  * "default" displays the Khulnasoft default background image.
...
* Customize the login page background image.
  ...
  * Using the CLI or a text editor:
    ...
    * Place custom image file in default or manual location:
      * Default destination folder: $KHULNASOFT_HOME/etc/apps/search/appserver/static/logincustombg.
        * Example: If your image is located at $KHULNASOFT_HOME/etc/apps/search/appserver/static/logincustombg/img.png, type loginCustomBackgroundImage = logincustombg/img.png.
      * Manual location: $KHULNASOFT_HOME/etc/apps/<myApp>/appserver/static/<pathToMyFile>, and type loginCustomBackgroundImage = <myApp:pathToMyFile>.
    * The login page background image updates automatically.
  * If no custom image is used, the default Khulnasoft background image displays.
...
* The expected file must be located at:
  $KHULNASOFT_HOME/etc/apps/<APP_NAME>/appserver/controllers/<PYTHON_NODULE_NAME>.py
* This module's methods will be exposed at /custom/<APP_NAME>/<PYTHON_NODULE_NAME>/<METHOD_NAME>

#
# exposed khulnasoftd REST endpoints
#
[expose:<unique_name>]
@placement search-head
* Registers a khulnasoftd-based endpoint that should be made available to the UI
  under the "/khulnasoftd" and "/khulnasoftd/__raw" hierarchies
* The name of the stanza doesn't matter as long as it starts with "expose:"
  Each stanza name must be unique, however

pattern = <url_pattern>
* Pattern to match under the khulnasoftd /services hierarchy.  For instance,
...
* Defaults to 0
* This is only needed for some internal khulnasoft endpoints, you probably
  should not specify this for app-supplied endpoints

skipCSRFProtection = [0 | 1]
* If set to 1, tells khulnasoftweb that it is safe to post to this endpoint
