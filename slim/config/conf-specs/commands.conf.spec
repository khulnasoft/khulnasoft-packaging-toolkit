...
# script to $KHULNASOFT_HOME/etc/searchscripts/ or
# $KHULNASOFT_HOME/etc/apps/MY_APP/bin/.  For the latter, put a custom
# commands.conf in $KHULNASOFT_HOME/etc/apps/MY_APP.  For the former, put your
# custom commands.conf in $KHULNASOFT_HOME/etc/system/local/.

# There is a commands.conf in $KHULNASOFT_HOME/etc/system/default/.  For examples,
# see commands.conf.example.  You must restart Khulnasoft to enable configurations.
...
* Set the following attributes/values for the command.  Otherwise, Khulnasoft uses
  the defaults.
* If the filename attribute is not specified, Khulnasoft searches for an
  external program by appending extensions (e.g. ".py", ".pl") to the
  stanza name.
* If chunked = true, in addition to ".py" and ".pl" as above, Khulnasoft
  searches using the extensions ".exe", ".bat", ".cmd", ".sh", ".js",
  and no extension (to find extensionless binaries).
* See the filename attribute for more information about how Khulnasoft
...
* Khulnasoft looks for the given filename in the app's bin directory.
* The filename attribute can not reference any file outside of the app's bin directory.
* If the filename ends in ".py", Khulnasoft's python interpreter is used
  to invoke the external script.
* If chunked = true, Khulnasoft looks for the given filename in
  $KHULNASOFT_HOME/etc/apps/MY_APP/<PLATFORM>/bin before searching
  $KHULNASOFT_HOME/etc/apps/MY_APP/bin, where <PLATFORM> is one of
  "linux_x86_64", "linux_x86", "windows_x86_64", "windows_x86",
  "darwin_x86_64" (depending on the platform on which Khulnasoft is
  running on).
...
* Additional command-line arguments to use when invoking this
  program. Environment variables will be substituted (e.g. $KHULNASOFT_HOME).
...
passauth = [true|false]
* If set to true, khulnasoftd passes several authentication-related facts
...
* The following headers are sent
  * authString: psuedo-xml string that resembles
    ...
    to contact khulnasoftd during the script run.
...
* NOTE: Should be set to true if you use khulnasoft.Interkhulnasoft
* Defaults to true.
...
* If true, multivalues will be treated as python lists of strings, instead of a
  flat string (when using Interkhulnasoft to interpret stdin/stdout).
...
is_risky = [true|false]
* Searches using Khulnasoft Web are flagged to warn users when they
...
* If false, this command uses the legacy custom search command
  protocol supported by Interkhulnasoft.py.
* Default is false
