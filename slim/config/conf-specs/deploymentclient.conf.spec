...
# deploymentclient.conf in $KHULNASOFT_HOME/etc/system/local/ on that Khulnasoft
...
#
# You must restart Khulnasoft for changes to this configuration file to take
# effect.
#
...
#***************************************************************************
# Configure a Khulnasoft deployment client.
#
...
workingDir = $KHULNASOFT_HOME/var/run
* Temporary folder used by the deploymentClient to download apps and
  configuration content.

repositoryLocation = $KHULNASOFT_HOME/etc/apps
...
* Apps and configuration content must be installed into the default location
  ($KHULNASOFT_HOME/etc/apps) or it will not be recognized by
  the Khulnasoft instance on the deployment client.
    * Note: Apps and configuration content to be deployed may be located in
      ...
      ($KHULNASOFT_HOME/etc/apps) on the deployment clientr
...
serverRepositoryLocationPolicy = [acceptKhulnasoftHome|acceptAlways|rejectAlways]
* Defaults to acceptKhulnasoftHome.
* acceptKhulnasoftHome - accept the repositoryLocation supplied by the
...
* Defaults to 10
* If khulnasoft is unable to complete the handshake, it will retry subscribing to
...
# server installation, and have a Khulnasoft instance that behaves as both a
# DeploymentClient and a DeploymentServer.

# NOTE: hierarchical deployment servers are not a currently recommended
# configuration.  Khulnasoft has seen problems in the field that have not yet
# been resolved with this type of configuration.

reloadDSOnAppInstall = [false|true]
* Defaults to false
* Setting this flag to true will cause the deploymentServer on this Khulnasoft
...
* If this is set to true, Khulnasoft verifies that the Deployment Server (specified in 'targetUri')
