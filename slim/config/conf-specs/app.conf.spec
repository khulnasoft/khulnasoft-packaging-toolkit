...
#   Version 20170103
#
# This file maintains the state of a given app in Khulnasoft Enterprise. It may also be used
# to customize certain aspects of an app.
#
# There is no global, default app.conf. Instead, an app.conf may exist in each
# app in Khulnasoft Enterprise.
#
# You must restart Khulnasoft Enterprise to reload manual changes to app.conf.
#
...
#
# Settings for how an app appears in Launcher (and online on Khulnasoftbase)
#
...
remote_tab = <bool>
* Set whether the Launcher interface will connect to apps.khulnasoft.com.
...
author = <name>
* For apps you intend to post to Khulnasoftbase, enter the username of your
  khulnasoft.com account.
...
# Launcher and Khulnasoftbase will automatically detect the images.
#
# For example:
#
...
# used in future versions of Khulnasoft Enterprise to streamline app upgrades.
#
...
* id should be omitted for internal-use-only apps which are not intended to be
  uploaded to Khulnasoftbase
* id is required for all new apps uploaded to Khulnasoftbase. Future versions of
  Khulnasoft Enterprise will use appid to correlate locally-installed apps and the
  same app on Khulnasoftbase (e.g. to notify users about app updates)
* id must be the same as the folder name in which your app lives in
  $KHULNASOFT_HOME/etc/apps
...
* Set whether Khulnasoft Enterprise should check Khulnasoftbase for updates to this app.
* Defaults to true.
...
* Set whether changing an app's state ALWAYS requires a restart of Khulnasoft Enterprise.
...
* Khulnasoft Enterprise will automatically populate this value upon install.
...
* was installed. Khulnasoft Enterprise will automatically populate this value upon
...
* Khulnasoft Enterprise will reload app configuration after every app-state change:
...
* If your app does not use a custom config file (e.g. myconffile.conf)
  ...
  $KHULNASOFT_HOME/etc/system/default/app.conf already includes a [triggers]
  stanza which automatically reloads config files normally used by Khulnasoft Enterprise.
* If your app uses a custom config file (e.g. myconffile.conf) and you want to
  avoid unnecessary Khulnasoft Enterprise restarts, you'll need to add a reload value in
  the [triggers] stanza.
* If you don't include [triggers] settings and your app uses a custom
  config file, a Khulnasoft Enterprise restart will be required after every state change.
* Specifying "simple" implies that Khulnasoft Enterprise will take no special action to
  reload your custom conf file.
* Specify "access_endpoints" and a URL to a REST endpoint, and Khulnasoft Enterprise will
  call its _reload() method at every app state change.
* Specify "http_get" and a URL to a REST endpoint, and Khulnasoft Enterprise will simulate
  an HTTP GET request against this URL at every app state change.
* Specify "http_post" and a URL to a REST endpoint, and Khulnasoft Enterprise will simulate
  an HTTP POST request against this URL at every app state change.
* "rest_endpoints" is reserved for Khulnasoft Enterprise internal use for reloading
  restmap.conf.

* Examples:

        [triggers]
        # Do not force a restart of Khulnasoft Enterprise for state changes of MyApp
        ...
        # Do not force a restart of Khulnasoft Enterprise for state changes of MyApp.
        # Khulnasoft Enterprise calls the /admin/myendpoint/_reload method in my custom
        # EAI handler.
...
label = <string>
* Defines the name of the app shown in the Khulnasoft Enterprise GUI and Launcher
* Recommended length between 5 and 80 characters.
* Must not include "Khulnasoft For" prefix.
* Label is required.
...
* This only applies to apps with documentation on the Khulnasoft documentation site
...
* For scripts, the command line should include both the interpreter and the
  script for it to run.
    * Example: "$KHULNASOFT_HOME/bin/python" "$KHULNASOFT_HOME/etc/apps/<myapp>/bin/$MY_SCRIPT"
* The invoked program is communicated with over standard in / standard out via
  the same protocol as khulnasoft scripted auth.
* Paths incorporating variable expansion or explicit spaces must be quoted.
  * For example, a path including $KHULNASOFT_HOME should be quoted, as likely
    will expand to C:\Program Files\Khulnasoft
...
* Setting this variable declares that this app will put additional information
  into the troubleshooting & support oriented output of the 'khulnasoft diag'
  command.
...
* Full discussion of the interface is located on the Developer portal.
  See http://dev.khulnasoft.com/view/SP-CAAAE8H
...
* This setting is new in Khulnasoft Enterprise/Light version 6.5.  Older versions
