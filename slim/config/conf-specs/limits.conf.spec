...
# search commands.
#
# There is a limits.conf in $KHULNASOFT_HOME/etc/system/default/.  To set custom
# configurations, place a limits.conf in $KHULNASOFT_HOME/etc/system/local/. For
# examples, see limits.conf.example. You must restart Khulnasoft to enable
# configurations.
#
...
#          are doing.  Improperly configured limits may result in khulnasoftd
...
* Messages logged to info.csv are available to REST API clients
  and the Khulnasoft UI, so limiting the messages
...
* Read more about subsearches in the online documentation:
  http://docs.khulnasoft.com/Documentation/Khulnasoft/latest/Search/Aboutsubsearches
...
* Maximum number of retries for creating a tmp directory (with random name as
  subdir of KHULNASOFT_HOME/var/run/khulnasoft)
* Defaults to 100.
...
interval = <integer>
* Number of seconds between logging khulnasoftd metrics to metrics.log.
* Minimum of 10.
* Defaults to 30.
...
* For each khulnasofttcp connection from forwarder, khulnasoft logs metrics information
...
* Intended as defense-in-depth against XSS style attacks against browser users
  by crafting specially encoded URLS for them to access khulnasoftd.
...
* How long search artifacts should be stored on disk once completed, in
  ...
  a job is being actively viewed in the Khulnasoft UI then the modtime of
...
* The number of search job status data khulnasoftd can cache in RAM. This cache
...
* Flag indicating whether configuration file replication blocks searches or is
  run asynchronously
* When setting this flag to auto Khulnasoft will choose to use asynchronous
...
* If set to true (1), Khulnasoft fetches all events accessible through the timeline from the remote
  ...
  * Fetching of all events may delay the finalization of some searches, typically those running in
    verbose mode from the main Search view in Khulnasoft Web.
...
* When set to "traditional", Khulnasoft initializes each search process completely from scratch
* When set to a string beginning with "debug", Khulnasoft routes searches through
  the given command, allowing the user the to "plug in" debugging tools
    ...
    * Khulnasoft will pass <debugging-args>, followed by the search command it
      would normally run, to <debugging-command>
    * For example, given:
        search_process_mode = debug $KHULNASOFT_HOME/bin/scripts/search-debugger.sh 5
      Khulnasoft will run a command that looks generally like:
        $KHULNASOFT_HOME/bin/scripts/search-debugger.sh 5 khulnasoftd search --id=... --maxbuckets=... --ttl=... [...]
* Defaults to "auto"
...
* When allowing more than one search to run per process, we'll try to reuse
  an idle process that last ran a search by the same Khulnasoft user
...
* When set to a negative value, we'll only allow a search process to be
  used by same Khulnasoft user each time
...
* On a search peer, allow remote (search head) bundles to be loaded in khulnasoftd.
* Defaults to false.
...
* Whether to use the dispatchtmp directory for temporary search time files
  ...
  directory).
* Temp files would be written to $KHULNASOFT_HOME/var/run/khulnasoft/dispatchtmp/<sid>/
...
check_khulnasoftd_period = <number>
...
* Do not use this setting without working in tandem with Khulnasoft support.
* This setting is a very narrow subset of summary_mode=none. When set to true, this
  ...
  khulnasoftd's /admin/summarization endpoint for report acceleration summary ids.
* In certain narrow use-cases this may improve performance if report acceleration
  (savedsearches.conf:auto_summarize) is not in use by lowering the main khulnasoftd's
  process overhead.
* Defaults to false.
...
* NOTE: Do not change unless instructed to do so by Khulnasoft Support.
...
* This is not a user tunable setting.  Do not use this setting without
  working in tandem with Khulnasoft personnel.  This setting is not tested at
  non-default.
* This controls whether the status cache is used, which caches information
  about search jobs (and job artifacts) in memory in main khulnasoftd.
...
* This is not a user tunable setting.  Do not use this setting without working
  in tandem with Khulnasoft personnel. This setting is not tested at non-default.
...
* Connection timeout for an indexer's search process when connecting to that
  indexer's khulnasoftd (in seconds)
* Defaults to 5
...
* Do not change this value unless directed by Khulnasoft support.
* Defaults to 90
...
* The memory of this input grows in size when the data being produced
  by applications writing to monitored files comes in faster than the Khulnasoft
...
* MIGRATION NOTE: if this setting doesn't exist, the initialization code in
  khulnasoftd triggers an automatic migration step that reads in the current value
...
* The maximum amount of time to defer running continuous scheduled searches
  ...
  This is used to prevent gaps in continuous scheduled searches when khulnasoftd
  was down.
...
* The maximum amount of time to run missed continuous scheduled searches for
  once Khulnasoft comes back up in the event it was down.
* Use [<int>]<unit> to specify a duration; a missing <int> defaults to 1.
...
* Maximum number of unexpired alerts information to keep for the alerts
  manager, when this number is reached Khulnasoft will start discarding the oldest
  alerts.
* Defaults to 50000

alerts_max_history = <integer>[s|m|h|d]
* Maximum time to search in the past for previously triggered alerts.
* khulnasoftd uses this property to populate the Activity -> Triggered Alerts page at startup.
* Defaults to 7 days.
* Values greater than the default may cause slowdown.

alerts_scoping = host|khulnasoft_server|all
* Determines the scoping to use on the search to populate the triggered alerts
  page. Choosing khulnasoft_server will result in the search query
  using khulnasoft_server=local, host will result in the search query using
  host=<search-head-host-name>, and all will have no scoping added to the
  search query.
* Defaults to khulnasoft_server.
...
* Khulnasoft will rarely return anywhere near this amount of results, as we will
...
* Default is 10000
* Do not change unless instructed to do so by Khulnasoft Support
...
* Internal setting, do not change unless instructed to do so by Khulnasoft Support
...
* Generate diagnostic WARN in khulnasoftd.log if reverse dns lookups are taking
...
* If not set, defaults to database included with khulnasoft
...
* Enables/disables automatic UI message notifications to admins for
  ...
  * Scheduled saved searches with invalid owners are considered "orphaned".  They
    cannot be run because Khulnasoft cannot determine the roles to use for the search
    context.
...
* Enables/disables automatic verification on every startup that all the files
  that were installed with the running Khulnasoft version are still the files that
  should be present.
  ...
  * The source of truth for the files that should be present is the manifest
    file in the $KHULNASOFT_HOME directory that comes with the release, so if this
...
* When "enabled" or "log_only", detected problems will cause details to be
  written out to khulnasoftd.log
