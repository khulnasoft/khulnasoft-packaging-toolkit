...
# server ("khulnasoft reload deploy-server"), or restart khulnasoftd, for changes to
...
* Can be overridden at the serverClass level.
* Defaults to $KHULNASOFT_HOME/etc/deployment-apps
...
* Defaults to $KHULNASOFT_HOME/etc/apps, the live
  configuration directory for a Khulnasoft instance.

tmpFolder = <path>
* Working folder used by deployment server.
* Defaults to $KHULNASOFT_HOME/var/run/tmp
...
* The value of this attribute is matched against several things in order:
    ...
    * For Khulnasoft version > 6.4, the instanceId of the client. This is a GUID
...
#     whitelist.0=*.khulnasoft.com
#     blacklist.0=printer.khulnasoft.com
#     blacklist.1=scanner.khulnasoft.com
# This will cause all hosts in khulnasoft.com, except 'printer' and 'scanner', to
# match this server class.

# Example with filterType=blacklist:
#     blacklist.0=*
#     whitelist.0=*.web.khulnasoft.com
#     whitelist.1=*.linux.khulnasoft.com
...
# type with this Khulnasoft CLI command on the DS:
#       <code>./khulnasoft list deploy-clients</code>
...
* The <pathname> is relative to $KHULNASOFT_HOME.
...
restartKhulnasoftWeb = true | false
* If true, restarts KhulnasoftWeb on the client when a member app or a directly
  configured app is updated.
* Can be overridden at the serverClass level and the serverClass:app level.
* Defaults to false

restartKhulnasoftd = true | false
* If true, restarts khulnasoftd on the client when a member app or a directly
  configured app is updated.
* Can be overridden at the serverClass level and the serverClass:app level.
* Defaults to false
