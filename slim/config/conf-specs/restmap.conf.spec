...
#
# There is a restmap.conf in $KHULNASOFT_HOME/etc/system/default/. To set custom
# configurations, place a restmap.conf in $KHULNASOFT_HOME/etc/system/local/. For
# help, see restmap.conf.example. You must restart Khulnasoft to enable
# configurations.
#
...
* Setting this to true, while convenient, may result in user/password getting
  logged as cleartext in Khulnasoft's logs *and* any proxy servers in between.
* Defaults to false.
...
* Defaults to $KHULNASOFT_HOME/bin/rest_handler.py.
...
* For example, $KHULNASOFT_HOME/etc/apps/<APPNAME>/bin/TestHandler.py has a class
  called MyHandler (which, in the case of python must be derived from a base
  class called 'khulnasoft.rest.BaseRestHandler'). The tag/value pair for this is:
...
* For scripttype=python this is optional.  It allows you to run a script
  which is *not* derived from 'khulnasoft.rest.BaseRestHandler'.  This is
...
* Whether configurations managed by this handler should be enumerated via the
  directory service, used by KhulnasoftWeb's "All Configurations" management page.
  Defaults to false.
