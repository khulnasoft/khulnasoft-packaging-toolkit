...
#
# There is a server.conf in $KHULNASOFT_HOME/etc/system/default/.  To set custom
# configurations, place a server.conf in $KHULNASOFT_HOME/etc/system/local/.
# For examples, see server.conf.example.  You must restart Khulnasoft to enable
# configurations.
#
...
* The name used to identify this Khulnasoft instance for features such as
  distributed search.
* Defaults to <hostname>-<user running khulnasoft>.
...
* If 'always', enables authentication so that all remote login attempts are
  allowed.
* If 'never', only local logins to khulnasoftd will be allowed. Note that this
  will still allow remote management through khulnasoftweb if khulnasoftweb is on
  the same server.
...
* Enables/disables logging to khulnasoftd_access.log for client phonehomes
...
* By default, khulnasoftd will listen for incoming connections (both REST and
  TCP inputs) using IPv4 only
* To enable IPv6 support in khulnasoftd, set this to 'yes'.  khulnasoftd will
  simultaneously listen for connections on both IPv4 and IPv6
* To disable IPv4 entirely, set this to 'only', which will cause khulnasoftd
...
* Note that any setting of KHULNASOFT_BINDIP in your environment or
  khulnasoft-launch.conf will override this value.  In that case khulnasoftd will
...
* If it is set to '6-first' then khulnasoftd will try IPv6 first and fallback to
  IPv4 on failure
* If this is set to '4-only' then khulnasoftd will only attempt to make
  connections over IPv4.
* Likewise, if this is set to '6-only', then khulnasoftd will only attempt to
...
* This setting now (as of 5.0) belongs in the [general] stanza of
  KHULNASOFT_HOME/etc/instance.cfg file; please see specfile of instance.cfg for
  more information.

useHTTPServerCompression = <bool>
* Whether khulnasoftd HTTP server should support gzip content encoding. For more
...
* Each rule can be in the following forms:
    ...
    3. A DNS name, possibly with a '*' used as a wildcard (examples:
       "myhost.example.com", "*.khulnasoft.com")
...
site = <site-id>
* Specifies the site that this khulnasoft instance belongs to when multisite is
  enabled.
* Valid values for site-id include site1 to site63

useHTTPClientCompression = true|false|on-http|on-https
* Whether gzip compression should be supported when Khulnasoftd acts as a client
...
* Data being loaded into khulnasoft, whether for indexing or forwarding,
...
* Informs components (such as the KhulnasoftWeb Manager section) which
  environment Khulnasoft is running in, to allow for more customized behaviors.
* Defaults to "download", meaning no special behaviors.

requireBootPassphrase = <bool>
* Prompt the user for a boot passphrase when starting Khulnasoft.
* Khulnasoft uses this passphrase to grant itself access to platform-provided
  secret storage facilities, like the GNOME keyring.
* For more information about secret storage, see the [secrets] stanza in
  $KHULNASOFT_HOME/etc/system/README/authentication.conf.spec.
...
* Allows 'khulnasoft clean eventdata' to clean the remote indexes when set to true.
* Defaults to false.
############################################################################
...
[sslConfig]
* Set SSL for communications on Khulnasoft back-end under this stanza name.
  * NOTE: To set SSL (eg HTTPS) for Khulnasoft Web and the browser, use
          web.conf.
* Follow this stanza name with any number of the following attribute/value
  pairs.
* If you do not specify an entry for each attribute, Khulnasoft will use the
  default value.

enableKhulnasoftdSSL = <bool>
* Enables/disables SSL on the khulnasoftd management port (8089) and KV store
  port (8191).
* Defaults to true.
* Note: Running khulnasoftd without SSL is not generally recommended.
...
* Enabling this potentially gives you much faster distributed searches
  across multiple Khulnasoft instances.
* Defaults to true.

useKhulnasoftdClientSSLCompression = <bool>
* Controls whether SSL compression would be used when khulnasoftd is acting as
...
* NOTE: this setting is effective if, and only if, useClientSSLCompression
        is set to true
* NOTE: khulnasoftd is not involved in data transfer in distributed search, the
...
* Comma-separated list of SSL versions to support for outgoing HTTP connections
  from khulnasoftd.  This includes distributed search, deployment client, etc.
...
* If this value is set, and 'sslVerifyServerCert' is set to true,
  khulnasoftd will limit most outbound HTTPS connections to hosts which use
...
* If this value is set, and 'sslVerifyServerCert' is set to true,
  khulnasoftd will also be willing to verify certificates which have a
...
* Requires that any HTTPS client that connects to khulnasoftd internal HTTPS
  server has a certificate that was signed by a CA (Certificate Authority)
  specified by 'sslRootCAPath'.
* Used by distributed search: Khulnasoft indexing instances must be
  authenticated to connect to another khulnasoft indexing instance.
...
* If set, Khulnasoft uses the specified cipher string for the HTTP server.
...
* The list of valid named curves by their short/long names can be obtained
  ...
  $KHULNASOFT_HOME/bin/khulnasoft cmd openssl ecparam -list_curves
...
* Certificates are auto-generated by khulnasoftd upon starting Khulnasoft.
* You may replace the default cert with your own PEM format file.
* Default is $KHULNASOFT_HOME/etc/auth/server.pem.
...
* Default is $KHULNASOFT_HOME/etc/auth.

certCreateScript = <script name>
* Creation script for generating certs on startup of Khulnasoft.
...
* This can help avoid a client being tricked later by a Man-In-The-Middle
  ...
  example, if khulnasoftweb is in default non-SSL mode this can break the
...
############################################################################
# Khulnasoftd HTTP server configuration
############################################################################

[httpServer]
* Set stand-alone HTTP settings for Khulnasoft under this stanza name.
* Follow this stanza name with any number of the following attribute/value
  pairs.
* If you do not specify an entry for each attribute, Khulnasoft uses the default
  value.
...
* If true, turns off listening on the khulnasoftd management port
  (8089 by default)
* This setting is not recommended:
  * This is the general communication path to khulnasoftd.  If it is disabled,
    there is no way to communicate with a running khulnasoft.
  * This means many command line khulnasoft invocations cannot function,
    khulnasoftweb cannot function, the REST interface cannot function, etc.
  * If you choose to disable the port anyway, understand that you are
    selecting reduced Khulnasoft functionality.
...
maxSockets = <int>
* The number of simultaneous HTTP connections that Khulnasoft Enterprise accepts
  simultaneously. You can limit this number to constrain resource usage.
* If set to 0, Khulnasoft Enterprise automatically sets it to one third of the
...
* If set to 0, Khulnasoft Enterprise automatically sets the limit to
...
* If maxSockets is not negative and maxThreads is greater than maxSockets, then
  Khulnasoft Enterprise sets maxThreads to be equal to maxSockets.
* If set to a negative number, no limit will be enforced.
* Defaults to 0.

keepAliveIdleTimeout = <int>
* How long, in seconds, that the Khulnasoftd HTTP server allows a keep-alive
...
* A list of the HTTP headers to which khulnasoftd sets
...
* Adds a X-Frame-Options header set to "SAMEORIGIN" to every response served by khulnasoftd
* Defaults to true

allowEmbedTokenAuth = true|false
* If set to false, khulnasoftd will not allow any access to artifacts
...
* This will be printed by the Khulnasoft CLI before it prompts
...
* Allows clients to make authenticated requests to the khulnasoft
...
* Defaults to true
* NOTE: has no effect if allowCookieAuth=false OR the khulnasoftd REST
...
############################################################################
# Khulnasoftd HTTPServer listener configuration
############################################################################

[httpServerListener:<ip:><port>]
* Enable the khulnasoftd REST HTTP server to listen on an additional port number
...
* Multiple "httpServerListener" stanzas can be specified to listen on
  more ports.
* Normally, khulnasoftd listens only on the single REST port specified in
...
* If the main REST port is SSL (the "enableKhulnasoftdSSL" setting in this
...
############################################################################
# Log rotation of khulnasoftd_stderr.log & khulnasoftd_stdout.log
############################################################################

# These stanzas apply only on UNIX.  khulnasoftd on Windows has no
# stdout.log or stderr.log

[stderr_log_rotation]
* Controls the data retention of the file containing all messages written to
  khulnasoftd's stderr file descriptor (fd 2).
* Typically this is extremely small, or mostly errors and warnings from
  linked libraries.

maxFileSize = <bytes>
* When khulnasoftd_stderr.log grows larger than this value, it will be rotated.
* maxFileSize is expresesed in bytes.
* You might want to increase this if you are working on a problem
  that involves large amounts of output to khulnasoftd_stderr.log
...
* How many rolled copies to keep.
  * For example, if this is 2, khulnasoftd_stderr.log.1 and khulnasoftd_stderr.log.2
    may exist.  Further rolls will delete the current khulnasoftd_stderr.log.2
* You might want to increase this if you are working on a problem
  that involves large amounts of output to khulnasoftd_stderr.log
* You might want to reduce this to allocate less storage to this log category.
* Defaults to 2.
...
* How often to check the size of khulnasoftd_stderr.log
...
[stdout_log_rotation]
* Controls the data retention of the file containing all messages written to
  khulnasoftd's stdout file descriptor (fd 1).
...
############################################################################
# Remote applications configuration (e.g. KhulnasoftBase)
############################################################################

[applicationsManagement]
* Set remote applications settings for Khulnasoft under this stanza name.
* Follow this stanza name with any number of the following attribute/value
  pairs.
* If you do not specify an entry for each attribute, Khulnasoft uses the default
  value.
...
* Allow Khulnasoft to access the remote applications repository.

url = <URL>
* Applications repository.
* Defaults to https://apps.khulnasoft.com/api/apps

loginUrl = <URL>
* Applications repository login.
* Defaults to https://apps.khulnasoft.com/api/account:login/

detailsUrl = <URL>
* Base URL for application information, keyed off of app ID.
* Defaults to https://apps.khulnasoft.com/apps/id

useragent = <khulnasoft-version>-<khulnasoft-build-num>-<platform>
...
updateHost = <URL>
* Host section of URL to check for app updates, e.g. https://apps.khulnasoft.com
...
* The minimum amount of time Khulnasoft will wait between checks for app updates
* Examples include '24h' (24 hours), '3d' (3 days),
  '7200s' (7200 seconds, or two hours)
* Defaults to '24h'

sslVersions = <versions_list>
* Comma-separated list of SSL versions to connect to 'url' (https://apps.khulnasoft.com).
* The versions available are "ssl3", "tls1.0", "tls1.1", and "tls1.2".
* The special version "*" selects all supported versions.  The version "tls"
  selects all versions tls1.0 or newer.
* If a version is prefixed with "-" it is removed from the list.
...
sslVerifyServerCert = <bool>
* If this is set to true, Khulnasoft verifies that the remote server (specified in 'url')
  being connected to is a valid one (authenticated).  Both the common
  name and the alternate name of the server are then checked for a
...
* The <path> must refer to a PEM format file containing one or more root CA
  certificates concatenated together.
* Used only if 'sslRootCAPath' is unset.
* Used for validating SSL certificate from https://apps.khulnasoft.com/

sslCommonNameToCheck = <commonName1>, <commonName2>, ...
* If this value is set, and 'sslVerifyServerCert' is set to true,
  khulnasoftd checks the common name(s) of the certificate presented by
  the remote server (specified in 'url') against this list of common names.
* Defaults to 'apps.khulnasoft.com'
...
sslAltNameToCheck =  <alternateName1>, <alternateName2>, ...
* If this value is set, and 'sslVerifyServerCert' is set to true,
  khulnasoftd checks the alternate name(s) of the certificate presented by
  the remote server (specified in 'url') against this list of subject alternate names.
* Defaults to 'khulnasoftbase.khulnasoft.com, apps.khulnasoft.com'
...
############################################################################
# Disk usage settings (for the indexer, not for Khulnasoft log files)
############################################################################
...
* Specifies a safe amount of space that must exist for khulnasoftd to continue
  operating.
* Note that this affects search and indexing
* For search:
  * Before attempting to launch a search, khulnasoft will require this amount of
    free space on the filesystem where the dispatch directory is stored,
    $KHULNASOFT_HOME/var/run/khulnasoft/dispatch
...
* For indexing:
  * Periodically, the indexer will check space on all partitions
    that contain khulnasoft indexes as specified by indexes.conf.  Indexing
    will be paused and a ui banner + khulnasoftd warning posted to indicate
...
############################################################################
# Settings controlling the behavior of 'khulnasoft diag', the diagnostic tool
############################################################################

[diag]

# These settings provide defaults for invocations of the khulnasoft diag
...
* Further excludes can be added at the khulnasoft diag command line, but there
  is no facility to disable configuration-based excludes at the command
  line.
* There is one exclude by default, for the khulnasoft.secret file.
...
* Normally, Khulnasoft diag will gather only three .DMP (crash dump) files on
  Windows to limit diag size.
* If this is set to true, khulnasoft diag will collect *all* .DMP files from
  the log directory.
...
* 'manifests' limits the index file-content collection to just
  .bucketManifest files which give some information about Khulnasofts idea of
...
* Files in the $KHULNASOFT_HOME/etc directory which are larger than this limit
...
* If desired, this filter may be entirely disabled by setting the value
  to 0.
* Currently, as a special exception, the file $KHULNASOFT_HOME?etc/system/replication/ops.json
...
* URI base to use for uploading files/diags to Khulnasoft support.
* If set to disabled (override in a local/server.conf file), effectively
  disables diag upload functionality for this Khulnasoft install.
* Modification may theoretically may permit operations with some forms of
  ...
  Request.
* The communication path with api.khulnasoft.com is over a simple but not
...
* Advanced setting.  Periodically the indexer must report to license manager
  ...
  memory + license_usage.log lines.  Set this only after consulting a Khulnasoft
...
#
#     $KHULNASOFT_HOME/etc/system/local/server.conf
#
...
# because Khulnasoft must use this value to find shared storage in the first
# place!
#
############################################################################
...
* Khulnasoft will store configurations and search artifacts here.
...
* For more information about update triggers specifically, see the
  [triggers] stanza in $KHULNASOFT_HOME/etc/system/README/app.conf.spec.
...
* If set to false, this search head will not run any update triggers. Note
  ...
  components within Khulnasoft that might care about those changes, like input
...
* Setting a value of "silent" is like setting a value of "true", with one
  ...
  signaled only by messages in khulnasoftd.log.
* Defaults to true.

lock.timeout = <time range string>
* Timeout for acquiring file-based locks on configuration files.
* Khulnasoft will wait up to this amount of time before aborting a configuration
  write.
...
* The information logged to the locked file is intended for debugging
  purposes only. Khulnasoft makes no guarantees regarding the contents of the
...
* Determines maximum time, in seconds, that a peer node waits for search
  ...
  in reponse to the 'khulnasoft offline' (or) 'khulnasoft offline --enforce-counts' command.
...
* Only valid for mode=master
* Enables/disables logging to khulnasoftd_access.log for peer heartbeats
...
* NOTE: you do not have to restart master to set this config parameter.
  Simply run the cli command on master:
    % khulnasoft edit cluster-config -access_logging_for_heartbeats <true|false>
...
* Do not change this setting without first consulting with Khulnasoft Support
...
* This is the address on which a slave will be available as search head.
  This is useful in the cases where a khulnasoft host machine has multiple
  interfaces and only one of them can be reached by another khulnasoftd
  instance.
...
* Only valid for mode=slave
* Khulnasoft will monitor each replication connection for presence of heartbeat,
...
* Khulnasoft will will close a replication connection if heartbeat is not seen
...
# Configure Khulnasoft to listen on a given TCP port for replicated data from
...
* For 'introspection_generator_addon', packaged with Khulnasoft; provides the
...
* This stanza controls the collection of i-data about:
  $KHULNASOFT_DB/fishbucket, where we persist per-input status of file-based
  inputs.
* Inherits the values of 'acquireExtra_i_data' and 'collectionPeriodInSecs'
...
* If you enable summary collection, the first collection happens 5 minutes
  after the Khulnasoft instance is started. For every subsequent collection, this
  setting is honored.
...
* Resource Usage i-data is collected at both hostwide and per-process
  levels; the latter, only for processes associated with this KHULNASOFT_HOME.
* Per-process i-data for Khulnasoft search processes will include additional,
...
* For 'introspection_generator_addon', packaged with Khulnasoft
...
############################################################################
# Settings used to control commands started by Khulnasoft
############################################################################

[commands:user_configurable]

prefix = <path>
* All non-internal commands started by khulnasoftd will be prefixed with this
...
* Does not apply to trusted/non-configurable command executions, such as:
  khulnasoft search, khulnasoft-optimize, gunzip.
...
* If SHPMaster info is switched on in log.cfg, then captain statistics will
  be dumped in khulnasoftd.log after the specified number of service periods.
...
* Unique identifier for this cluster as a whole, shared across all cluster
  members.
* By default, Khulnasoft will arrange for a unique value to be generated and
...
* If true, Khulnasoft will log the the low-level heartbeats between members in
  khulnasoftd_access.log  . These heartbeats are used to maintain the authority
...
* Enables/disables logging to khulnasoftd_access.log for member heartbeats
* Defaults to false (logging disabled)
* NOTE: you do not have to restart captain to set this config parameter.
  Simply run the cli command on master:

  % khulnasoft edit shcluster-config -access_logging_for_heartbeats <true|false>
...
* Do not alter this value without contacting khulnasoft support.
* Defaults to 10000
...
* Controls whether Khulnasoft replicates changes to a particular type of *.conf
...
* Any individual file within a configuration replication summary that is
  larger than this value (in MB) will trigger a khulnasoftd.log warning message.
* Defaults to 50.
...
* In other words: controls the minimum number of configuration changes
  Khulnasoft will remember for replication purposes.
* Defaults to 20000.
...
* Any individual file within <conf_deploy_repository> that is larger than
  this value (in MB) will trigger a khulnasoftd.log warning message.
* Defaults to: 50
...
artifact_status_fields = <field> ...
    ...
    * These fields will be shows in cli/rest endpoint khulnasoft list shcluster-member-artifacts
...
* This setting has no effect on a single Khulnasoft instance.
...
* Changing this directory after initial startup does not move existing data.
  ...
  location.
* Defaults to $KHULNASOFT_DB/kvstore.
...
* If specified, it will be used in KV Store SSL connections and
  authentication.
* Only used when Common Criteria is enabled (KHULNASOFT_COMMON_CRITERIA=1)
  or FIPS is enabled (i.e. KHULNASOFT_FIPS=1).
* NOTE: Khulnasoft plans to submit Khulnasoft Enterprise for Common Criteria
  evaluation. Khulnasoft does not support using the product in Common
  Criteria mode until it has been certified by NIAP. See the "Securing
  Khulnasoft Enterprise" manual for information on the status of Common
  Criteria certification.
* Default is $KHULNASOFT_HOME/etc/auth/cacert.pem
...
* Must be specified if FIPS is enabled (i.e. KHULNASOFT_FIPS=1), otherwise, KV
