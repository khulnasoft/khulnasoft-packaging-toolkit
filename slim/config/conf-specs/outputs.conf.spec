#   Version 20170103
#
# Forwarders require outputs.conf; non-forwarding Khulnasoft instances do not
# use it.  It determines how the forwarder sends data to receiving Khulnasoft
# instances, either indexers or other forwarders.
#
# To configure forwarding, create an outputs.conf file in
# $KHULNASOFT_HOME/etc/system/local/.  For examples of its use, see
...
# http://docs.khulnasoft.com/Documentation/Khulnasoft/latest/Deploy/Aboutforwardingandreceivingdata
...
* Takes a comma separated list of one or more systems to send data to over a
  tcp socket.
* Typically used to specify receiving khulnasoft systems, although it can be
  used to send data to non-khulnasoft systems (see sendCookedData setting).
...
token = <string>
* Optional
* If an access token is configured for receiving Khulnasoft system, that token
...
* If true, events are cooked (have been processed by Khulnasoft).
...
* This value essentially determines how many active-but-old channels we keep
  "pinned" in memory on both sides of a khulnasoft-to-khulnasoft connection.
...
* If set to -1, the TcpOutputProcessor will make sure that each group will
  get all of the events.  If one of the groups is down, then Khulnasoft will
  block everything.
* Defaults to 5.
...
* You should not normally need to change these filters from their default
  settings in $KHULNASOFT_HOME/system/default/outputs.conf.
...
* The default Khulnasoft CAcert uses the password "password".
...
maxEventSize = <integer>
* If specified, sets the maximum size of an event that khulnasoft will transmit.
...
# Edit $KHULNASOFT_HOME/etc/system/local/props.conf and set a TRANSFORMS-routing
...
# data on full Khulnasoft. It has the "index" property, which determines whether
# indexing occurs.
#
# When Khulnasoft is not configured as a forwarder, "index" is set to "true".
# That is, the Khulnasoft instance indexes data by default.
#
# When Khulnasoft is configured as a forwarder, the processor turns "index" to
# "false". That is, the Khulnasoft instance does not index data by default.
#
...
* Default depends on whether the Khulnasoft instance is configured as a
...
* Example: https://KhulnasoftMaster01.example.com:8089
