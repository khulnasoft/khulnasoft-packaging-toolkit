...
#
# There is an eventdiscoverer.conf in $KHULNASOFT_HOME/etc/system/default/.  To set
# custom configurations, place an eventdiscoverer.conf in
# $KHULNASOFT_HOME/etc/system/local/.  For examples, see
# eventdiscoverer.conf.example. You must restart Khulnasoft to enable
# configurations.
#
...
* For more details, refer to $KHULNASOFT_HOME/etc/system/default/eventdiscoverer.conf).
* Default = "sun, mon, tue,..." 

ignored_fields = <comma-separated list of fields>
* Similar to ignored_keywords, except these are fields as defined in Khulnasoft
  instead of terms.
...
* For the full default setting, see $KHULNASOFT_HOME/etc/system/default/eventdiscoverer.conf.
