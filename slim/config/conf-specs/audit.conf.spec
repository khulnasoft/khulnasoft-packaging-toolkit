...
# audit.conf in $KHULNASOFT_HOME/etc/system/local/. For examples, see
# audit.conf.example.  You must restart Khulnasoft to enable configurations.
#
...
* WARNING: Enabling signing for audit trail events may increase HTTP response latency if
           the Khulnasoft instance is handling large numbers of concurrent requests.

privateKey= <path>
* The path to the file containing the private key.
* Generate your own keys using openssl in $KHULNASOFT_HOME/bin/.
* If not present, a default key will be generated one time and placed at
  $KHULNASOFT_HOME/etc/auth/audit/private.pem

publicKey= <path>
* The path to the file containing the public key.
* Generate your own keys using openssl in $KHULNASOFT_HOME/bin/.
* If not present, a default key will be generated one time and placed at
  $KHULNASOFT_HOME/etc/auth/audit/public.pem
