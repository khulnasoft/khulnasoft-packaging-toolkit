...
def main(args):
    ...
    slim.utils.SlimLogger.step(
        "Extracting source from ", slim.utils.encode_filename(args.source), "..."
    )
    app_source = get_app_source(args.source, None)
    server_collection = get_app_server_class_collection(
        args.installation, args.repository
    )

    if args.partition_only:
        if args.target_os:
            slim.utils.SlimLogger.information(
                '"target-os: %s" is ignored because --partition-only is used.'
                % args.target_os
            )
    else:
        slim.utils.SlimLogger.step(
            "Adding ", app_source.qualified_id, " to installation graph..."
        )

        server_collection.add(
            app_source,
            get_deployment_specifications(
                args.deployment_packages,
                args.combine_search_head_indexer_workloads,
                args.forwarder_workloads,
            ),
            args.target_os,
        )
        ...
        slim.utils.SlimLogger.information(
            "Saved updated installation graph to ", slim.utils.encode_filename(filename)
        )
...
def partition(source, installation_graph, output_dir):
    ...
    slim.utils.SlimLogger.step(
        "Extracting source from ", slim.utils.encode_filename(source), "..."
    )

    app_source = get_app_source(source, None)
    server_collection = get_app_server_class_collection(
        installation, slim.utils.slim_configuration.repository_path
    )
...
def _partition(app_source, server_collection, output_dir, partition_all):
    ...
    deployment_packages = server_collection.partition(
        app_source, output_dir, partition_all
    )

    if len(deployment_packages) > 0:
        app_id = app_source.qualified_id
        slim.utils.SlimLogger.information(
            "Generated deployment packages for ",
            app_id,
            ":\n  ",
        ...
        slim.utils.SlimLogger.information(
            "No deployment packages generated for ",
            app_source.id,
            " because there is nothing meaningful to deploy",
        )
...
def get_deployment_specifications(
    ...
    deployment_specifications = (
        slim.app.AppDeploymentSpecification.get_deployment_specifications(
            deployment_specifications,
            combine_search_head_indexer_workloads,
            forwarder_deployment_specifications,
        )
