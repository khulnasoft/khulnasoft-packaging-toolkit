...
parser.add_argument(
    ...
    dest="update",
    action="store_true",
    help="amend the current app manifest based on configuration " "settings.",
)
...
def main(args):
    try:
        ...
        if (
            SlimLogger.error_count()
            and args.output.ostream != sys.stdout
            and not args.output.pre_existing
        ):
...
def update_manifest(source, output):
    ...
    SlimLogger.step(
        "Loading app manifest from ", encode_filename(app_manifest_filename), "..."
    )
    app_manifest = AppManifest.load(app_manifest_filename)
    SlimLogger.exit_on_error()

    SlimLogger.step(
        "Updating app manifest from app configuration to ",
        encode_filename(output.name),
        "...",
    )
