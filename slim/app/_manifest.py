    ...
    class Converter(JsonDataTypeConverter):
        ...
        def convert_from(self, data_type, value):
            ...
            def convert(name, version_spec):
                ...
                raise ValueError(
                    'Version requirement includes no supported version of Khulnasoft ' + name + ': ' + string(version_spec))
...
class AppTargetWorkloadsConverter(JsonDataTypeConverter):
    ...
    def convert_from(self, data_type, value):
        ...
        if value not in self.targets:
            raise ValueError('Expected a Khulnasoft deployment target, not ' + encode_string(value))
        return value
...
class AppKhulnasoftReleaseInfo(object):
    ...
    def load(cls, file=None):
        if file is None:
            file = path.join(slim_configuration.system_config, 'khulnasoft-releases.json')
        if isinstance(file, string):
    ...
    schema = JsonSchema('Khulnasoft release info', JsonValue(
        JsonObject(
            any=JsonValue(
...
class AppKhulnasoftRequirement(ObjectView):
    ...
    def __str__(self):
        return ', '.join(('Khulnasoft ' + edition + ' edition ' + string(self[edition]) for edition in self))
        ...
        def convert_from(self, data_type, value):
            ...
            def convert(name, version_spec):
                try:
                    ...
                    raise ValueError('Expected a Khulnasoft edition name, not ' + encode_string(name))
                if version_spec is None:
                    return name, None
                for version in versions:
                    if version in version_spec:
            ...
            return AppKhulnasoftRequirement((convert(name, value[name]) for name in value))
...
class AppDeploymentConverter(JsonDataTypeConverter):
    ...
    def convert_from(self, data_type, value):
        ...
        if value not in self.deployments:
            raise ValueError('Expected a Khulnasoft deployment type, not ' + encode_string(value))
        return value
...
"""
    ...
    schema = JsonSchema('manifest', JsonValue(required=True, data_type=JsonObject(
        ...
        JsonField('platformRequirements', JsonObject(
            JsonField('khulnasoft', JsonObject(any=JsonValue(
                JsonString(), converter=JsonVersionSpecConverter(), required=True
