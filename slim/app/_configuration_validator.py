...
class AppConfigurationValidator(object):
        ...
        try:
            ...
            if count == 0:
                SlimLogger.warning(
                    "Could not find ",
                    basename,
                    " on configuration_spec_path:\n  ",
                    "\n  ".join(configuration_spec_dirs),
                )
                cls._configuration_specs[
                    configuration
                ] = configuration_spec = cls._NoConfigurationSpec
            else:
    ...
    def _default_stanza_validator(self):
        ...
        def validate(setting):
            ...
            SlimLogger.warning(
                setting.position,
                ": Undefined setting in stanza [default]: ",
                setting.name,
            )
    ...
    def _non_default_stanza_validator(self, stanza):
        ...
        if stanza_declarations is None:
            SlimLogger.warning(
                stanza.position,
                ": Setting validation is disabled for [",
                stanza.name.replace("\n", "\\n"),
                "] " "because there is no matching configuration spec",
            )
            return AppConfigurationValidator._validate

        stanza_declarations = OrderedSet(
            stanza_declarations + configuration_spec.match("default")
        )
        ...
        def validate(setting):
            ...
            SlimLogger.warning(
                setting.position,
                ": Undefined setting in ",
                configuration_spec.name,
                ".conf, stanza [",
                stanza.name.replace("\n", "\\n"),
                "]: ",
                setting.name,
