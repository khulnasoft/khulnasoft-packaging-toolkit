...
class AppDependencyGraph(Mapping):
    ...
    def __init__(
        ...
        target_os=SlimTargetOSWildcard,
    ):
        ...
        self._repository_sources = app_source.populate_dependency_sources(
            path.abspath(repository), installed_packages
        )
    ...
    def print_description(self, ostream):
        ...
        Example:
        [dependency-graph]
        |-- com.khulnasoft.app:fictional@1.0.0
    ...
    def export_source_package(self, output_dir):
        ...
        with TarFile.gzopen(
            source_package_path, mode="w", compresslevel=9, encoding="utf-8"
        ) as source_package:

            source_package.add(
                source.directory,
                arcname=app_name,
                filter=SlimIgnore(app_name, source.directory).filter,
            )
            ...
            if len(self._graph) > 1:
                ...
                for dependency in self._graph:
                    ...
                    source_package.add(
                        package,
                        arcname=path.join(".dependencies", path.basename(package)),
                    )
    ...
    def get_deployment_specifications(self, deployment_specification):
        ...
        if (
            input_groups is None
            or input_groups == AppDeploymentSpecification.all_input_groups
        ):
            # Each app receives the same deployment specification in the absence of input groups
            result = OrderedDict(
                ((source, deployment_specification) for source in self._graph)
            )
        else:
            # Deployment specifications vary based on the input group requirements of the root app, transitively
            result = OrderedDict()
            self._get_deployment_specifications(
                self._root, deployment_specification, result
            )
    ...
    def report_unreferenced_input_groups(self, level):
        ...
        for app_source in self._graph:  # pylint: disable=too-many-nested-blocks
            ...
            referenced_dependencies = OrderedDict(
                (app_dependency, frozenset()) for app_dependency in app_dependencies
            )
            ...
            if dependency_declarations is not None:
                for group_name in input_groups:
                    ...
                    for dependency_name in group_requires:
                        dependency_package = dependency_declarations[
                            dependency_name
                        ].package
                        ...
                        referenced_dependencies[dependency_source] = union_of(
                            references, dependency_groups
                        )
                    ...
                    if count > 0:
                        if count == 1:
                            ...
                            SlimLogger.message(
                                ...
                                ": unreferenced input group: dependency ",
                            ...
                            SlimLogger.message(
                                level,
                                app_source.qualified_id,
                                ": unreferenced input groups: dependency ",
                                dependency_source.qualified_id,
                                ": \n    ",
                                "\n    ".join(unreferenced),
                            )
    ...
    def traverse(self, visit):
        ...
        while len(queue) > 0:
            ...
            if app_source not in visited:
                ...
                dependency_sources = visit(
                    app_source, dependency_sources, dependent_sources
                )
    ...
    def _add_source(self, app_source):
        ...
        while len(queue) > 0:
            ...
            if app_source not in graph:
                app_source._dependencies = app_dependencies = self._get_dependencies(
                    app_source
                )
            ...
            for dependency, dependent_source in dependents:
                ...
                if not version_range.match(version):
                    SlimLogger.error(
                        ...
                        ": Version ",
                        version,
                        " was selected, but version ",
                        version_range,
                        " is " "required by ",
                        dependent_source.qualified_id,
                    )
    ...
    def _describe(self, app_source, level):
        ...
        if level == 1:
            graph_output += (
                "|-- " + app_source.id + "@" + string(app_source.version) + "\n"
            )
    ...
    def _get_dependencies(self, app_source):
        ...
        if app_source.manifest.dependencies is not None:
            for name, dependency in app_source.get_dependencies_for_target_os(
                self._target_os
            ):
                ...
                if dependency.package:
                    ...
                    SlimLogger.warning(
                        "Skipping validation for optional dependency ",
                ...
                try:
                    dependency_source = dependency_sources[
                        package
                    ]  # from app source .dependencies
                except KeyError:
                    try:
                        dependency_source = repository_sources[
                            package
                        ]  # from repository (backup)
                    except KeyError:
                        SlimLogger.error(
                            "Expected to find static dependency ",
                ...
                if version_range.match(version) is False:
                    SlimLogger.error(
                        app_source.id,
                        ": Packaged version of dependency ",
                        encode_string(name),
                        " is outside range ",
                        version_range,
                        ": ",
                        encode_filename(dependency_source.package),
                    )
    ...
    def _get_deployment_specifications(
        self, app_source, deployment_specification, result
    ):
        ...
        if input_groups is not None:
            ...
            for name in deployment_specification.inputGroups:
                ...
                for alias in aliases:
                    ...
                    assert (
                        dependency_source in self._graph[app_source]
                    ), "Dependency graph is corrupt"
                    requirements[dependency_source] = union_of(
                        requirements.get(
                            dependency_source,
                            AppDeploymentSpecification.no_input_groups,
                        ),
                        requirement,
                    )
        ...
        for dependency, dependency_source in app_source.dependencies:
            input_groups = requirements.get(
                dependency_source, AppDeploymentSpecification.no_input_groups
            )
                ...
                dependency_deployment_specification = AppDeploymentSpecification(
                    (
                        ...
                        (
                            "inputGroups",
                            union_of(
                                result[dependency_source].inputGroups, input_groups
                            ),
                        ),
                    )
                )
            ...
            self._get_deployment_specifications(
                dependency_source, dependency_deployment_specification, result
            )
    ...
    def _union_of(fg_1, fg_2):
        return (
            AppDeploymentSpecification.all_input_groups
            if "*" in fg_1 or "*" in fg_2
            else fg_1.union(fg_2)
        )
...
class AppDeploymentPackage(object):
    ...
    def __init__(self, app_source, deployment_specification):
        ...
        group, name, version = (
            app_id.group,
            app_root if app_id.name is None else app_id.name,
            app_id.version,
        )

        self._name = "-".join(
            (part for part in (group, name, string(version)) if part is not None)
        )
        ...
        for configuration_file in app_configuration.files():
            ...
            package = (
                InputsPackagingRule
                if configuration_file.name == "inputs"
                else DefaultPackagingRule
            ).instance()
            ...
            for section in configuration_file.sections():
                ...
                for stanza in section.stanzas():
                    ...
                    for setting in stanza.settings():
                        if package.should_include_setting(
                            stanza, setting, deployment_specification, app_manifest
                        ):
                            relevant_settings[setting.name] = setting
                    if package.should_include_stanza(
                        stanza,
                        relevant_settings,
                ...
                if len(relevant_stanzas) > 0:
                    file_name = section.name[
                        len(path.commonprefix((app_root, section.name))) + 1 :
                    ]
        ...
        self._is_empty = self._detect_is_empty(
            app_source, self._asset_filenames, relevant_configurations
        )
    ...
    def __repr__(self):
        result = (
            ...
            + repr(self._name)
            + "stage_name="
            + repr(self._stage_name)
            + "archive_name="
            + repr(self._archive_name)
            + "configuration="
            + repr(self._configuration)
            + ")"
        )
        return result
    ...
    def export(self, output_dir):
        ...
        if not is_debug_enabled:
            ...
            SlimLogger.debug(
                "Saved ",
                encode_filename(self._stage_name),
                " configuration to ",
            ...
            def append_digest(tar_info):
                if not tar_info.isdir():
                    ...
                    digest.append(
                        OrderedDict(
                            (("name", name), ("objectId", object_id), ("size", size))
                        )
                    )
                return tar_info
    ...
    def _detect_is_empty(app_source, asset_filenames, configurations):
        ...
        if len(configurations) == 0 or (
            len(configurations) == 1 and "app" in configurations
        ):
    ...
    _exclusion_rule = (
        ...
        (
            ("README", _exclude_conf_spec),
            frozenset(("forwarder", "indexer", "searchHead")),
        ),
    )
    ...
    def _get_match_function(self, pattern):
        ...
        return pattern.__get__(
            self, self.__class__
        )  # expectation: pattern is a member function

    def _ignore_assets(self, source, names):
        return [
            name
            for name in names
            if path.join(source, name) not in self._asset_filenames
        ]
    ...
    def _set_asset_filenames(self, ignore_patterns):
        ...
        for root, directory_names, filenames in os.walk(
            app_root, topdown=True, followlinks=True
        ):
            app_subdir = split_filename(
                root[len(path.commonprefix((app_root, root))) + 1 :]
            )
            if len(directory_names) > 0:
                for name in get_excluded_filenames(
                    app_subdir, directory_names, ignore_patterns
                ):
            ...
            if len(filenames) > 0:
                for name in get_excluded_filenames(
                    app_subdir, filenames, ignore_patterns
                ):
        ...
        empty_directories = deque(
            (name for name in file_counts if file_counts[name] == 0)
        )
...
class AppDeploymentSpecification(ObjectView):
        ...
        for input_group in forwarder_workloads:
            ...
            def update():
                ...
                if deployment_specification is None:
                    ...
                    input_groups = deployment_specification.inputGroups
                    deployment_specification["inputGroups"] = input_groups.union(
                        (input_group,)
                    )

                if server_class in ("_search_heads", "_indexers"):
                    workload = deployment_specification.workload
                    deployment_specification["workload"] = workload.union(
                        (server_class,)
                    )
            ...
            if isinstance(server_classes, list):
                for server_class in server_classes:
                    if isinstance(server_class, string):
                        ...
                        continue
                    raise ValueError(
                        "Expected a server class name, not ",
                        ObjectView.get_json_type_name(server_class),
                        ": ",
                        server_class,
                    )
                continue
    ...
    def get_deployment_specifications(
        ...
        combine_search_head_indexer_workloads,
        forwarder_deployment_specifications,
    ):
        if combine_search_head_indexer_workloads:
            ...
            if len(deployment_specifications) == 0:
                ...
                deployment_specifications.append(
                    AppDeploymentSpecification(
                        (
                ...
                for index, deployment_specification in enumerate(
                    deployment_specifications
                ):
                    if deployment_specification.workload.isdisjoint(
                        ("searchHead", "indexer")
                    ):
                        continue
                    if deployment_specification.inputGroups is None:
                        continue
                    input_groups = input_groups.union(
                ...
                if len(input_groups) == 0:
                    ...
                    deployment_specification = AppDeploymentSpecification(
                        (
                            ...
                            (
                                "workload",
                                frozenset(("searchHead", "indexer", "forwarder")),
                            ),
                            ("inputGroups", input_groups),
                        )
                    )
            ...
            if (
                len(deployment_specifications) == 0
                and forwarder_deployment_specifications is None
            ):
                ...
                for deployment_specification in deployment_specifications:
                    workload = deployment_specification.workload.intersection(
                        update_list
                    )
                    count = len(workload)
        ...
        for deployment_specification in deployment_specifications:
            ...
            if name in name_clashes:
                SlimLogger.error(
                    "Duplicate deployment specification name: ", encode_string(name)
                )
    ...
    class SafeFilenameConverter(JsonDataTypeConverter):
        def convert_from(self, data_type, value):
            ...
            if match is None:
                raise ValueError(
                    "Illegal characters in deployment specification.name "
                    + encode_string(value)
                    + " which must "
        ...
        JsonValue(
            ...
            data_type=JsonObject(
                JsonField(
                    "name",
                    JsonString(),
                    converter=SafeFilenameConverter(),
                    required=True,
                ),
                JsonField(
                    "workload",
                ...
                JsonField(
                    "inputGroups",
                    JsonArray(JsonValue(JsonString())),
                    converter=InputGroupsConverter(),
                    required=False,
                ),
            ),
            converter=Converter(),
