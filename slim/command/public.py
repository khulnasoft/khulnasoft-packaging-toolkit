...
class SetDebugAction(Action):
    def __init__(
        self, option_strings, dest, help=None, metavar=None
...
class SetOutputDirAction(Action):
    ...
    def __init__(self, option_strings, dest, help=None, metavar=None):
        Action.__init__(
            ...
            option_strings,
            dest,
            type=SlimDirectoryArgument(),
            default=slim_configuration.output_dir,
            help=help,
            metavar=metavar,
            nargs=1,
        )
...
class SetRepositoryAction(Action):
    ...
    def __init__(self, option_strings, dest, help=None, metavar=None):
        Action.__init__(
            ...
            dest,
            type=SlimDirectoryArgument(),
            default=slim_configuration.repository_path,
            help=help,
            metavar=metavar,
            nargs=1,
        )
...
class SlimArgumentError(ArgumentError):
    def __init__(self, *args):
        ArgumentError.__init__(
            self, argument=None, message="".join((string(x) for x in args))
        )


class SlimArgumentParser(ArgumentParser):
    def __init__(
        self, add_help=False, fromfile_prefix_chars="@", *args, **kwargs
    ):  # nopep8, pylint: disable=keyword-arg-before-vararg
    ...
    def add_app_directory(self):
        self._options.add_argument(
            "source",
            type=SlimDirectoryArgument(existent=True),
    ...
    def add_installation(self):
        ...
        return self._options.add_argument(
            "-i",
            "--installation",
            type=SlimFileArgument(encoding="utf-8"),
            default=default,
    ...
    def add_output_directory(self, description):
        return self._options.add_argument(
            ...
            help="save "
            + description
            + " to the directory at this location (default: current directory)",
            metavar="<output-dir>",
        )
    ...
    def add_output_file(self, description, manifest=False):
        if manifest:
            return self._options.add_argument(
                "-o",
                "--output",
                type=SlimManifestFileArgument("a"),
                default=SlimManifestFileArgument.default(),
                help="save the "
                + description
                + " to the file at this location (default: stdout)",
                metavar="<filename>",
            )
        return self._options.add_argument(
            "-o",
            "--output",
            type=SlimFileArgument("wb"),
            default=sys.stdout,
            help="save the "
            + description
            + " to the file at this location (default: stdout)",
    ...
    def add_unreferenced_input_groups(self):
        return self._options.add_argument(
            "-u",
            "--unreferenced-input-groups",
            type=string,
            choices=frozenset(SlimUnreferencedInputGroups._fields),
            default=SlimUnreferencedInputGroups.info,
            help="""
    ...
    def add_combine_search_head_indexer_workloads(self):
        return self._options.add_argument(
            "-c",
            "--combine-search-head-indexer-workloads",
            action="store_const",
            const=True,
            default=False,
    ...
    def add_forwarder_workloads(self):
        return self._options.add_argument(
            "-f",
            "--forwarder-workloads",
            type=SlimForwarderWorkloadsArgument(),
            default=None,
            help="""
    ...
    def add_deployment_packages(self):
        return self._options.add_argument(
            "-d",
            "--deployment-packages",
            type=SlimDeploymentSpecificationArgument(),
            nargs="+",
            default=[],
    ...
    def add_target_os(self):
        return self._options.add_argument(
            "-t",
            "--target-os",
            type=SlimTargetOSArgument(),
            default=SlimTargetOSWildcard,
            help="specify a target OS: %s, or %s (default: %s)"
            % (", ".join(SlimTargetOS[:-1]), SlimTargetOS[-1], SlimTargetOSWildcard),
            metavar="<target-os>",
        )
        ...
        if not path.isdir(value):
            ...
            try:
                ...
                raise SlimArgumentError(
                    ...
                    + error.strerror
                    + ": "
                    + encode_filename(error.filename)
                )
        return value
...
class SlimFileArgument(object):
    ...
    def __call__(self, value):
        ...
        try:
            ...
            message = "cannot open {0}: {1}".format(
                encode_filename(value), error.strerror
            )
            raise SlimArgumentError(message)
...
class SlimManifestFileArgument(SlimFileArgument):
    ...
    def _init_ostream(self):
        try:
            ...
            message = "cannot open {0}: {1}".format(
                encode_filename(self._value), error.strerror
            )
            raise SlimArgumentError(message)
...
class SlimForwarderWorkloadsArgument(object):
    def __call__(self, value):
        ...
        try:
            ...
            raise SlimArgumentError(
                "Poorly formed forwarder workloads: ", error, ": ", value
            )
    ...
    def __call__(self, value):
        try:
            ...
            try:
                ...
                raise SlimArgumentError(
                    "Expected a source package or source directory name: ", value
                )
