...
class SlimConfigurationManager(object):
    ...
    def configuration_spec_path(self):
        return self._get_path_option(
            "_configuration_spec_path", "configuration_spec_path"
        )
    ...
    def get(setting_names, location=None, validate=False):
        ...
        files = (
            list(cls._files.values()) if location is None else [cls._files[location]]
        )  # nopep8, pylint: disable=unsubscriptable-object
        ...
        for setting_name in setting_names:
            ...
            section, option = keys
            if (
                section in cls._defaults and option in cls._defaults[section]
            ):  # nopep8, pylint: disable=unsupported-membership-test,unsubscriptable-object
    ...
    def load(self):
        ...
        self._configuration_spec_path = (
            self._repository_path
        ) = self._temp_directory_path = None  # set on first access
        self._settings = SlimConfigurationManager._create_config_parser(
            SafeConfigParser, list(cls._files.values())
        )

        SlimLogger.set_level(
            cls._defaults["logger"]["level"]
        )  # pylint: disable=unsubscriptable-object
    ...
    def set(settings, location=None):
        ...
        for setting_name, setting_value in settings.items():
            ...
            if len(value) == 2:
                ...
                if (
                    parser.has_section(section) and option in defaults[section]
                ):  # nopep8, pylint: disable=unsubscriptable-object
        ...
        try:
            ...
            if validate:
                ...
                extra_sections = [
                    ...
                    if section
                    not in slim_sections  # nopep8, pylint: disable=unsupported-membership-test
                ]
                if len(extra_sections) > 0:
                    SlimLogger.warning(
                        "ignoring unrecognized section names: ",
                        encode_series(extra_sections),
                    )
                ...
                for section, slim_options in SlimConfigurationManager._defaults.items():
                    ...
                    if len(extra_options) > 0:
                        SlimLogger.warning(
                            "ignoring unrecognized options for section [",
                            section,
                            "]: ",
    ...
    def _initialize(system_home=None):
        ...
        cls._defaults = OrderedDict(
            (
                (
                    "logger",
                    OrderedDict((("level", logging.getLevelName(logging.STEP)),)),
                ),
                (
                    "option",
                    OrderedDict(
                        (
                            (
                                "configuration_spec_path",
                                path.join(cls._slim_config, "conf-specs"),
                            ),
                            (
                                "repository_path",
                                path.join(cls._user_config, "repository"),
                            ),
    ...
    def _save(parser, filename):
        ...
        with io.open(filename, encoding="utf-8", mode="w", newline="") as stream:
            for section in parser.sections():
                ...
                for name in SlimConfigurationManager._defaults[
                    section
                ]:  # nopep8, pylint: disable=unsubscriptable-object
                    try:
