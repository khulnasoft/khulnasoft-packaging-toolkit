...
class SlimIgnore(object):
    def __init__(self, app_name, source_directory):
        ...
        if len(includes) == 0:
            ...
            def _filter(item):
                value = (
                    None
                    if SlimIgnore.Item(self, item).match(excludes) is True
                    else item
                )
                return value
    ...
    def _compile_patterns(ordered_dict):
        pattern = "|".join(
            "(?P<" + t + string(n) + ">" + p + ")"
            for n, (p, t) in enumerate(ordered_dict.items())
        )
        ...
        for line in SlimIgnore._read_lines(source_directory):
            ...
            if len(names) == 1:
                ...
                for i, name in enumerate(names[start:], start):
                    ...
                    if name == "**":
                        ...
                        names[i] = SlimIgnore._wildcard.sub(
                            r"[^" + escaped_path_sep + "]*", name
                        )
                ...
                if (
                    first_name == "**"
                ):  # TODO: Should the comparison not be len(first_name) == 0 or first_name == '**'
            ...
            try:
                ...
                if pattern_type == "f" and current_pattern_type == "d":
                    del target[
                        pattern
                    ]  # remove the currently-defined pattern from our targeted dictionary
                    target[
                        pattern
                    ] = pattern_type  # add the new pattern to the end of our targeted dictionary
        ...
        for slimignore_file in (
            ...
            try:
                ...
                if error.errno != 2:  # no such file
                    SlimLogger.fatal(
                        "Could not open ", encode_filename(slimignore_file), ": ", error
                    )
    ...
    class Item(object):
        def __init__(self, slim_ignore, item):
            ...
            if item_type is string:
                ...
                source_pathname = (
                    slim_ignore._source_pathname
                )  # pylint: disable=protected-access
                common_prefix = path.commonprefix([source_pathname, name])
                self._name = path.normcase(
                    None
                    if len(common_prefix) == len(name)
                    else name[len(common_prefix) + 1 :]
                )
            else:
