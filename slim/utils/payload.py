...
class SlimPayload(object):
    ...
    def payload(self):
        final_payload = OrderedDict(
            (("status", self._status), ("messages", self._messages))
        )
    ...
    def set_supported_deployments(self, supported_deployments):
        self._specific_payload.setdefault("manifest", OrderedDict())[
...
class SlimPayloadLoggingHandler(logging.Handler):
    def __init__(self, payload=None):
        ...
        if not isinstance(payload, SlimPayload):
            raise TypeError(
                "Expected payload of type SlimPayload, not " + type(payload).__name__
            )
    ...
    def payload(self, value):
        if not isinstance(value, SlimPayload):
            raise TypeError(
                "Expected payload of type SlimPayload, not " + type(value).__name__
            )
        self.acquire()
        try:
            self._payload = value
